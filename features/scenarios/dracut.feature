...
Feature: NM: dracut
    ...
    * Run dracut test
    ...
    @not_on_ppc64le @skip_in_centos
    @dracut @long
    @dracut_NM_NFS_root_nfs_fqdn
    Scenario: NM - dracut - NM module - NFSv3 root=nfs with FQDN
    * Run dracut test
      | Param  | Value                                                                  |
      | kernel | root=nfs:nfs-server.nfs.redhat.com:/client ro ip=dhcp                  |
      | qemu   | -device virtio-net,guest_csum=off,netdev=nfs,mac=52:54:00:12:34:00     |
      | qemu   | -netdev tap,id=nfs,script=$PWD/qemu-ifup/nfs                           |
      | check  | nmcli_con_active "Wired Connection" eth0                               |
      | check  | nmcli_con_prop "Wired Connection" ipv4.method auto                     |
      ...
      | check  | ip6_route_unique "deaf:beef::1:10 dev eth0 proto kernel"               |
      | check  | ip6_route_unique "deaf:beef::/64 dev eth0 proto ra"                    |
      | check  | nfs_server nfs-server.nfs.redhat.com                                   |
    ...
    * Run dracut test
    ...
    @rhbz1879014
    @rhelver+=9.1 @rhelver-=9 @fedoraver-=0
    @ver+=1.41.4
    @not_on_ppc64le @skip_in_centos
    @dracut @long
    ...
    * Run dracut test
    ...
    # rhel8.3-rhel9: uses ip= kernel arg to pin bridge MAC and prevent DHCP lease exhaustion
    @rhbz1879003
    @rhelver+=8.3 @rhelver-=9 @fedoraver-=0
    @ver+=1.27
    @not_on_ppc64le @skip_in_centos
    @dracut @long
    ...
    * Run dracut test
      ...
      | kernel | ip=br0.0033:dhcp:1500:52:54:00:12:34:16                                  |
      | qemu   | -netdev tap,id=vlan33_0,script=$PWD/qemu-ifup/vlan33_0                   |
    ...
    # rhel10+: no ip= kernel arg needed, bridge MAC is stable
    @rhbz1879003
    @rhelver+=10 @fedoraver+=38
    @ver+=1.27
    @not_on_ppc64le @skip_in_centos
    @dracut @long
    @dracut_NM_vlan_over_bridge
    ...
    * Run dracut test
    ...
    @rhbz2092215
    @rhelver+=8.3 @rhelver+=9.1 @rhelver-=9 @fedoraver-=0
    @ver+=1.27
    @ver-1.40
    @ver+=1.41.4
    @not_on_ppc64le @skip_in_centos
