Feature: nmcli: clat
    ...
    * Bring "up" connection "testX-clat"
    * Verify the CLAT connection over device "testX"
    Then "clat inet4 192.0.0.5 inet6 3fff:aaa::.* pref64 2001:db8:122:344::/64" is visible with command "nmcli"
    Then "IP4.CLAT-ADDRESS:.*192.0.0.5" is visible with command "nmcli connection show testX-clat"
    Then "IP6.CLAT-ADDRESS:.*3fff:aaa::.*" is visible with command "nmcli connection show testX-clat"
    ...
    * Prepare a CLAT environment on device "testX" with NAT64 prefix "2001:db8::/32" and IPv6 prefix "3fff:aaa::"
    * Prepare a CLAT environment on device "testY" with NAT64 prefix "2001:db9:100::/40" and IPv6 prefix "3fff:bbb::"
    * Prepare a CLAT environment on device "testZ" with NAT64 prefix "2001:dba:111::/48" and IPv6 prefix "3fff:ccc::"
    ...
    * Prepare a CLAT environment on device "testX" with NAT64 prefix "64:ff9b::/96" and IPv6 prefix "3fff:aaa::"
    * Add "ethernet" connection named "testX-clat" for device "testX" with options
          """
          ipv4.clat no
          autoconnect no
    ...
    * Bring "up" connection "testX-clat"
    ...
    Then Check "ipv6" address list "/3fff:aaa::[0-9a-f:]+/64 /fe80::[0-9a-f:]+/64" on device "testX" in "10" seconds
    * Ignore possible AVC "bpf"
