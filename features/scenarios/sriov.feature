 Feature: nmcli: sriov
    ...
    * Prepare "99-sriov.conf" config for "p4p1" device with "0" VFs
    ...
    When "1" is visible with command "nmcli dev |grep p4p1  |wc -l"
    * Prepare "99-sriov.conf" config for "p4p1" device with "2" VFs
    * Add "ethernet" connection named "sriov" for device "p4p1_0" with options "autoconnect no ipv4.method manual ipv4.address 1.2.3.4/24"
    * Add "ethernet" connection named "sriov_2" for device "p4p1" with options "autoconnect no ipv4.method manual ipv4.address 1.2.3.5/24"
    ...
    * Bring "up" connection "sriov_2"
    ...
    When "1" is visible with command "nmcli dev |grep p4p1  |wc -l"
    * Prepare "99-sriov.conf" config for "p4p1" device with "2" VFs
    * Add "ethernet" connection named "sriov" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24"
    * Add "ethernet" connection named "sriov_2" for device "p4p1" with options "ipv4.method manual ipv4.address 1.2.3.5/24"
    When "p4p1_0" is visible with command "nmcli device" in "5" seconds
    Then "activated" is visible with command "nmcli -g GENERAL.STATE con show sriov" in "5" seconds
     And "activated" is visible with command "nmcli -g GENERAL.STATE con show sriov_2" in "5" seconds
    ...
    * Execute "ip add add 192.168.1.2/24 dev p4p1_0"
    ...
    # * Prepare "99-sriov.conf" config for "p4p1" device with "2" VFs
    # * Add "ethernet" connection named "sriov" for device "enp5s16f1" with options "ipv4.method manual ipv4.address 1.2.3.4/24 802-3-ethernet.mtu 9000"
    ...
    * Bring "up" connection "sriov"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24 autoconnect no"
    * Bring "up" connection "sriov_2"
    ...
    * Add "ethernet" connection named "sriov_2" for device "''" with options " match.interface-name p4p1_* connection.multi-connect multiple "
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:99 trust=true' sriov.total-vfs 1 autoconnect no"
    * Bring "up" connection "sriov"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24 autoconnect no"
    * Bring "up" connection "sriov_2"
    Then "1" is visible with command "cat /sys/class/net/p4p1/device/sriov_numvfs"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:99 trust=true' sriov.total-vfs 1 802-3-ethernet.mtu 9000 autoconnect no"
    * Bring "up" connection "sriov"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24 802-3-ethernet.mtu 9000 autoconnect no"
     * Bring "up" connection "sriov_2"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 vlans=100.2.q' sriov.total-vfs 1 autoconnect no"
    * Bring "up" connection "sriov"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24 autoconnect no"
    ...
    * Execute "pkill tcpdump"
    ...
    # * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 vlans=100.2.ad' sriov.total-vfs 1"
    # * Bring "up" connection "sriov"
    # * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24"
    # * Run child "tcpdump -n -i p4p1 -xxvv -e > /tmp/tcpdump.log"
    # When "empty" is not visible with command "file /tmp/tcpdump.log" in "20" seconds
    # * Bring "up" connection "sriov_2"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:99 trust=true' sriov.total-vfs 1 "
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:99 trust=false' sriov.total-vfs 1"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24"
    When " connected" is visible with command "nmcli  device |grep p4p1_0"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:55 spoof-check=false' sriov.total-vfs 1"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24"
    When " connected" is visible with command "nmcli  device |grep p4p1_0"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:55 spoof-check=true' sriov.total-vfs 1"
    # * Bring "up" connection "sriov"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24"
    When " connected" is visible with command "nmcli  device |grep p4p1_0"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 vlans=100.2.q' sriov.total-vfs 1 autoconnect no"
    * Bring "up" connection "sriov"
    * Add "ethernet" connection named "sriov_2" for device "p4p1_0" with options "ipv4.method manual ipv4.address 1.2.3.4/24 connection.zone work autoconnect no"
    * Bring "up" connection "sriov_2"
    Then "1" is visible with command "cat /sys/class/net/p4p1/device/sriov_numvfs"
    And " connected" is visible with command "nmcli  device |grep p4p1_0"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:55 trust=true' sriov.total-vfs 1"
    * Add "bond" connection named "sriov_bond0" for device "sriov_bond" with options "ipv4.method manual ipv4.address 1.2.3.4/24 bond.options 'mode=active-backup,primary=p4p1_0,miimon=100,fail_over_mac=2'"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.total-vfs 1 sriov.autoprobe-drivers false"
    Then "1" is visible with command "cat /sys/class/net/p4p1/device/sriov_numvfs"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:99' sriov.total-vfs 1 sriov.autoprobe-drivers false"
    Then "1" is visible with command "cat /sys/class/net/p4p1/device/sriov_numvfs"
    ...
    * Run child "tcpdump -n -i p4p1 -xxvv -e > /tmp/tcpdump.log"
    When "empty" is not visible with command "file /tmp/tcpdump.log" in "20" seconds
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 vlans=100.2.q' sriov.total-vfs 1 sriov.autoprobe-drivers false"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 trust=false' sriov.total-vfs 1 sriov.autoprobe-drivers false"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:55 spoof-check=false' sriov.total-vfs 1 sriov.autoprobe-drivers false"
    ...
    * Add "ethernet" connection named "sriov" for device "p4p1" with options "sriov.vfs '0 mac=00:11:22:33:44:55 spoof-check=true' sriov.total-vfs 1 sriov.autoprobe-drivers false"
