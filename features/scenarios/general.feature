...
## Basically various bug related reproducer tests follow here
    ...
    * Bring "up" connection "testeth0"
    * Cleanup execute "iptables -t nat -D POSTROUTING -o eth0 -j MASQUERADE"
    ...
    * Execute "ip netns exec peers ip route add default via 172.16.0.1"
    ...
    Then "1" is visible with command "cat /proc/sys/net/ipv4/conf/vethbrg/forwarding" in "5" seconds
    * Bring "down" connection "con_general"
    Then "1" is visible with command "cat /proc/sys/net/ipv4/conf/eth0/forwarding" in "5" seconds
