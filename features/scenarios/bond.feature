 Feature: nmcli: bond
    ...
    * Add "ethernet" connection named "bond0.1" for device "eth4" with options "master nm-bond"
    ...
    @ver+=1.55.3
    @ver/rhel/9+=1.54.4.1
    @bond_non_8023ad_no_lacp_active_on_reapply
    Scenario: nmcli - bond - no lacp_active warning for non-802.3ad bond on reapply
     * Add "bond" connection named "bond0" for device "nm-bond" with options
           """
           ip4 172.16.1.1/24
...
#FIXME: more tests with arp and conflicts with load balancing can be written
    ...
    * Start dnsmasq for "bond" in namespace "ns1" with options "-d --interface bond1 --except-interface lo --host-record=client1234,172.25.1.101 --log-queries --no-resolv --server=8.8.8.8"
    ...
    * Cleanup file "/tmp/bond_ifindex"
    * Cleanup file "/tmp/bond_dbus_path"
    * Cleanup file "/tmp/dhcp4_config_path"
    ...
    * Execute "ip -o l show nm-bond | cut -d: -f1 > /tmp/bond_ifindex"
    Then "dhcp_client_identifier" is visible with command "cat /run/NetworkManager/devices/$(cat /tmp/bond_ifindex)"
    * Execute "nmcli -g DBUS-PATH,DEVICE device | grep nm-bond | cut -d: -f1 > /tmp/bond_dbus_path"
    * Execute "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/bond_dbus_path) org.freedesktop.NetworkManager.Device Dhcp4Config | awk '{print $2}' | tr -d '\"' > /tmp/dhcp4_config_path"
    Then "dhcp_client_identifier" is visible with command "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/dhcp4_config_path) org.freedesktop.NetworkManager.DHCP4Config Options"
    * Note the output of "nmcli -t -f DHCP4.OPTION d show nm-bond | grep dhcp_client_identifier | cut -d= -f2" as value "client_id_nmcli"
    * Note the output of "grep dhcp_client_identifier /run/NetworkManager/devices/$(cat /tmp/bond_ifindex) | cut -d= -f2" as value "client_id_run"
    * Note the output of "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/dhcp4_config_path) org.freedesktop.NetworkManager.DHCP4Config Options | grep dhcp_client_identifier | sed 's/.*\"dhcp_client_identifier\" s \"\([^\"]*\)\".*/\1/'" as value "client_id_dbus"
    ...
    * Cleanup file "/tmp/bond_ifindex"
    * Cleanup file "/tmp/bond_dbus_path"
    * Cleanup file "/tmp/dhcp6_config_path"
    ...
    * Execute "ip -o l show nm-bond | cut -d: -f1 > /tmp/bond_ifindex"
    Then "dhcp6_client_id" is visible with command "cat /run/NetworkManager/devices/$(cat /tmp/bond_ifindex)"
    * Execute "nmcli -g DBUS-PATH,DEVICE device | grep nm-bond | cut -d: -f1 > /tmp/bond_dbus_path"
    * Execute "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/bond_dbus_path) org.freedesktop.NetworkManager.Device Dhcp6Config | awk '{print $2}' | tr -d '\"' > /tmp/dhcp6_config_path"
    Then "dhcp6_client_id" is visible with command "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/dhcp6_config_path) org.freedesktop.NetworkManager.DHCP6Config Options"
    * Note the output of "nmcli -t -f DHCP6.OPTION d show nm-bond | grep dhcp6_client_id | cut -d= -f2" as value "duid_nmcli"
    * Note the output of "grep dhcp6_client_id /run/NetworkManager/devices/$(cat /tmp/bond_ifindex) | cut -d= -f2" as value "duid_run"
    * Note the output of "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/dhcp6_config_path) org.freedesktop.NetworkManager.DHCP6Config Options | grep dhcp6_client_id | sed 's/.*\"dhcp6_client_id\" s \"\([^\"]*\)\".*/\1/'" as value "duid_dbus"
    ...
    * Execute "ip -o l show nm-bond | cut -d: -f1 > /tmp/bond_ifindex"
    Then "iaid" is visible with command "cat /run/NetworkManager/devices/$(cat /tmp/bond_ifindex)"
    * Execute "nmcli -g DBUS-PATH,DEVICE device | grep nm-bond | cut -d: -f1 > /tmp/bond_dbus_path"
    * Execute "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/bond_dbus_path) org.freedesktop.NetworkManager.Device Dhcp6Config | awk '{print $2}' | tr -d '\"' > /tmp/dhcp6_config_path"
    Then "iaid" is visible with command "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/dhcp6_config_path) org.freedesktop.NetworkManager.DHCP6Config Options"
    * Note the output of "nmcli -t -f DHCP6.OPTION d show nm-bond | grep iaid | cut -d= -f2" as value "iaid_nmcli"
    * Note the output of "grep iaid /run/NetworkManager/devices/$(cat /tmp/bond_ifindex) | cut -d= -f2" as value "iaid_run"
    * Note the output of "busctl get-property org.freedesktop.NetworkManager $(cat /tmp/dhcp6_config_path) org.freedesktop.NetworkManager.DHCP6Config Options | grep iaid | sed 's/.*\"iaid\" s \"\([^\"]*\)\".*/\1/'" as value "iaid_dbus"
    ...
    * Execute "ip link set dummy1 down"
    ...
    Scenario: bond - check GARP message in balance-slb mode when L3 bridged
    * Add "bridge" connection named "br1" for device "br1" with options "stp off ipv4.method static ipv4.address 192.168.99.1/24 ipv6.method static ipv6.address 2026::feed:bee/64"
    * Add "bond" connection named "bond1" for device "bond1" with options "bond.options mode=balance-xor,xmit_hash_policy=vlan+srcmac,balance-slb=1 controller br1"
    * Add "dummy" connection named "dummy1" for device "dummy1" with options "controller bond1"
    * Add "dummy" connection named "dummy2" for device "dummy2" with options "controller bond1"
    * Note MAC address output for device "bond1" via ip command as "bond_mac"
    * PCAP "bond1" interface
    ...
    * Execute "ip link set dummy1 down"
    ...
    Scenario: bond - check RARP message in balance-slb mode when L2 bridged
    * Add "bridge" connection named "br1" for device "br1" with options "stp off ipv4.method disabled ipv6.method disabled"
    * Add "bond" connection named "bond1" for device "bond1" with options "bond.options mode=balance-xor,xmit_hash_policy=vlan+srcmac,balance-slb=1 controller br1"
    * Add "dummy" connection named "dummy1" for device "dummy1" with options "controller bond1"
    * Add "dummy" connection named "dummy2" for device "dummy2" with options "controller bond1"
    * Note MAC address output for device "bond1" via ip command as "bond_mac"
