Feature: nmcli: ipv4
    ...
    * Commentary
      ...
      """
    * Execute "ip -n testX_ns addr add dev testXp 172.25.10.1/24"
    * Run child "ip netns exec testX_ns dnsmasq --bind-interfaces --interface testXp -d --port 0 --dhcp-range=172.25.10.100,172.25.10.200,60 --dhcp-option=108,8"
    ...
    * Create NM config file "90-ipv6-only-preferred.conf" with content
      ...
      """
    * Restart NM
    * Prepare simulated test "testX" device with "192.168.12" ipv4 and daemon options "--dhcp-option=108,1800"
    * Prepare simulated test "testY" device with "192.168.13" ipv4 and daemon options "--dhcp-option=108,1800"
    * Prepare simulated test "testZ" device with "192.168.14" ipv4 and daemon options "--dhcp-option=108,1800"
