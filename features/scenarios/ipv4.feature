Feature: nmcli: ipv4
    ...
    * Execute "ln -s /var/lib/NetworkManager/internal-$(nmcli -f connection.uuid -t con show id con_ipv4 | sed 's/.*://')-eth2.lease /tmp/ipv4_client_id.lease"
    ...
    * Execute "sed 's/CLIENTID=.*/CLIENTID=00000000000000000000000000000000000000/' < /tmp/ipv4_client_id.lease.copy > /tmp/ipv4_client_id.lease"
    ...
    * Commentary
    ...
    """
    * Modify connection "con2" changing options "ipv4.add-dns-routes yes"
    * Modify connection "con3" changing options "ipv4.add-dns-routes yes"
    * Bring "up" connection "con2"
    * Bring "up" connection "con3"
    Then "via 172.16.1.254 dev eth2" is visible with command "ip route get 192.1.1.1"
    Then "via 172.16.2.254 dev eth3" is visible with command "ip route get 192.2.2.2"
    Then "fwmark 0x4e4d3030 lookup 1313681456" is visible with command "ip rule show prio 22000"
    Then "2" is visible with command "ip route show table 1313681456 | wc -l"

    * Commentary
    """
    Check that live reapply works
    """
    * Execute "nmcli device modify eth3 ipv4.add-dns-routes no"
    Then "via 172.16.1.254 dev eth2" is visible with command "ip route get 192.2.2.2"
    * Execute "nmcli device modify eth3 ipv4.add-dns-routes yes"
    Then "via 172.16.2.254 dev eth3" is visible with command "ip route get 192.2.2.2"
    ...
    * Bring "down" connection "con3"
    Then "0" is visible with command "ip route show table 1313681456 | wc -l"
    ...
    * Create NM config file "10-add-dns-routes.conf" with content
    """
    [connection-add-dns-routes]
    ipv4.add-dns-routes=1
    """
    * Restart NM
