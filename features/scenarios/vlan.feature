Feature: nmcli - vlan
    ...
    * Execute "for i in $(seq 10 510); do nmcli con add type vlan con-name eth11.$i id $i dev eth11 ethernet.cloned-mac-address random  ipv4.dhcp-timeout 120 ipv4.may-fail no ipv4.dad-timeout 0 ipv6.method disable; sleep 0.25; done"
    ...
    When "\s+activated" is visible with command "for i in 1 2; do nmcli con show eth11."$i"33; done" in "200" seconds
    When "\s+activated" is visible with command "for i in 3 4; do nmcli con show eth11."$i"77; done" in "200" seconds
    When "\s+activated" is visible with command "for i in 4 5; do nmcli con show eth11."$i"99; done" in "200" seconds
    When "\s+activated" is visible with command "for i in 2 3; do nmcli con show eth11."$i"55; done" in "200" seconds
    When "\s+activated" is visible with command "nmcli con show eth11.101" in "200" seconds
    When "\s+activated" is visible with command "nmcli con show eth11.202" in "200" seconds
    When "\s+activated" is visible with command "nmcli con show eth11.303" in "200" seconds
    When "\s+activated" is visible with command "nmcli con show eth11.404" in "200" seconds
    When "\s+activated" is visible with command "nmcli con show eth11.505" in "200" seconds
    # We need to see eth11 + 501 vlans so we are sure we have all
    When Execute "nmcli  device |grep eth11 > /tmp/eth11s"
    When "502" is visible with command "nmcli  device |grep eth11 |wc -l" in "20" seconds
