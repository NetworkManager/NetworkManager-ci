    ...
    * Start dnsmasq for "ip6" with options "--no-resolv --bind-interfaces -i test11 --enable-ra --dhcp-range=::1,::400,constructor:test11,ra-only,64,15s"
    ...
    * Load nftables in "testX6_ns" namespace
        ...
        """
    * Start dnsmasq for "ip6" in namespace "dhcp_ns" with options "--log-dhcp --log-queries=extra --interface=dhcp --leasefile-ro --dhcp-range=2620:dead:beaf::,static,ra-only --dhcp-host=id:00:11:22,[::1234:5678],static"
    ...
    * Execute "for if in testX6p dhcpp dupp; do ip -n testX6_ns link set $if master br0 ; done"
    * Execute "for if in testX6p dhcpp dupp br0 ; do ip -n testX6_ns link set $if up ; done"
    * Start dnsmasq for "ip6" in namespace "dhcp_ns" with options "--log-dhcp --log-queries=extra --interface=dhcp --enable-ra --dhcp-range=2620:dead:beaf::,slaac"
    * Run child "ip netns exec testX6_ns tshark -n -l -i br0 'icmp6 or port 547 or port 546' > /tmp/tshark.log"
    * Execute "ip link set testX6 up"
    ### uncomment following steps to verify that address is assigned correctly in absence of duplicate address
    #* Bring "up" connection "con_ipv6"
    ...
    * Commentary
    ...
    """
    * Modify connection "con2" changing options "ipv6.routed-dns yes"
