...
def check_pattern_command(
    ...
    while xtimeout.loop_sleep(interval):
        ...
        if check_type == "default":
            ...
            assert (
                ret == 0
            ), 'Pattern "%s" disappeared after %s seconds, ouput was:\n%s' % (
                pattern,
                xtimeout.elapsed_time(),
                proc.before,
            )
        elif check_type == "not_full":
            assert (
                ret != 0
            ), 'Pattern "%s" appeared after %s seconds, output was:\n%s%s' % (
                pattern,
                xtimeout.elapsed_time(),
                proc.before,
                proc.after,
...
@step(u'Do not expect "{pattern}" in children in "{seconds}" seconds')
def not_expect_children(context, pattern, seconds):
    seconds = float(seconds)
    for child in nmci.pexpect.pexpect_service_find_all("child"):
        proc = child.proc
        r = proc.expect([pattern, nmci.pexpect.EOF, nmci.pexpect.TIMEOUT], timeout=seconds)
        assert r != 0, f"Child {proc.name} has '{pattern}' in output:\n{proc.before}{proc.after}"
...
def check_routes_expect(context, ifname, addr_family, expected):
    ...
    conf = devices[0][f"ip{nmci.ip.addr_family_num(addr_family)}config"]

    routes = conf["_routes"]

    try:
        nmci.util.compare_strv_list(expected, routes, ignore_order=True)
    except ValueError as e:
        print(">>> about to fail check_routes_expect()")
        os.system(f"ip -{nmci.ip.addr_family_num(addr_family)} route show table all")
