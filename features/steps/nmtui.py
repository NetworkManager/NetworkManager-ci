...
def check_process_not_running(context):
    assert context.tui.isalive() is False, "NMTUI (pid:%s) is still up!" % context.tui.pid
...
def set_specific_field_to(context, field, value):
    ...
    if "Profile name" in field:
        nmci.cleanup.cleanup_add_connection(value)
    elif "Device" in field:
        nmci.cleanup.cleanup_add_iface(value)
