...
## Basically various bug related reproducer tests follow here
    ...
    * Execute "cd nmstate && ulimit -n 10000 && pytest -vv tests/integration -k 'not test_dhcp_on_bridge0 | test_bond_with_a_slave  | test_take_over_virtual_interface_then_remove | test_take_over_virtual_interface_and_rollback | test_create_and_remove_vlan | add_bond_with_slaves_and_ipv4' --log-level=DEBUG 2>&1 | tee /tmp/nmstate.txt"
