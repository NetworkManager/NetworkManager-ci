...
def create_policy_based_routing_files(context, profile, dev, table):
    ips = context.command_output(
        "nmcli connection sh %s |grep IP4.ADDRESS |awk '{print $2}'" % profile
    )
    ip_slash_prefix = ips.split("\n")[0]
    ip = ip_slash_prefix.split("/")[0]
    gw = context.command_output(
        "nmcli connection sh %s |grep IP4.GATEWAY |awk '{print $2}'" % profile
    ).strip()
    ...
    context.command_code(
        "echo 'prio 17201 iif %s table %s' > /etc/sysconfig/network-scripts/rule-%s"
        % (dev, table, profile)
    )
    context.command_code(
        "echo 'prio 17200 from %s table %s' >> /etc/sysconfig/network-scripts/rule-%s"
        % (ip, table, profile)
    )
    time.sleep(1)
...
@step(
    u'Prepare pppoe server for user "{user}" with "{passwd}" password and IP "{ip}" authenticated via "{auth}"'
)
...
@step(
    u"Restart dhcp server on {device} device with {ipv4} ipv4 and {ipv6} ipv6 dhcp address prefix"
)
...
@step(
    u'Prepare simulated test "{device}" device using dhcpd and server identifier "{server_id}"'
)
def prepare_dhcpd_simdev(context, device, server_id):
    ipv4 = "192.168.99"
    context.command_code("ip netns add {device}_ns".format(device=device))
    context.command_code(
        "ip link add {device} type veth peer name {device}p".format(device=device)
    )
    context.command_code(
...
@step(
    u'Prepare simulated test "{device}" device with "{ipv4}" ipv4 and "{ipv6}" ipv6 dhcp address prefix and dhcp option "{option}"'
)
@step(
    u'Prepare simulated test "{device}" device with "{ipv4}" ipv4 and "{ipv6}" ipv6 dhcp address prefix'
)
@step(
    u'Prepare simulated test "{device}" device with "{ipv4}" ipv4 and daemon options "{daemon_options}"'
)
...
@step(
    u'Prepare simulated test "{device}" device with daemon options "{daemon_options}"'
)
def prepare_simdev(
    context,
    device,
    ...
    if lease_time != "infinite":
        dnsmasq_command += " --dhcp-range={ipv6}::100,{ipv6}::fff,slaac,64,{lease_time} \
                             --enable-ra".format(
            lease_time=lease_time, ipv6=ipv6
        )
...
@step(
    u'Prepare simulated test "{device}" device with DHCPv4 server on different network'
)
def prepare_simdev(context, device):
    ...
    context.command_code(
        "ip link add {device}2 type veth peer name {device}2p".format(device=device)
    )
    context.command_code(
        "ip link set {device}p netns {device}_ns".format(device=device)
    )
...
def prepare_simdev(context, device):
    ...
    context.pexpect_service(
        ...
        [
            "-c",
            "ip netns exec {device}2_ns nc -6 -l -p 8080 > /dev/null".format(
                device=device
            ),
        ],
    )
...
def prepare_simdev_no_carrier(context, device):
    ...
    context.command_code(
        "ip netns exec {device}_ns ip link set {device} up".format(device=device)
    )
    context.command_code(
...
def start_pppoe_server(context, name, ip, dev):
    ...
    context.pexpect_service(
        ...
        [
            "-c",
            "kill -9 $(pidof pppoe-server); ip netns exec %s_ns pppoe-server -S %s -C %s -L %s -p /etc/ppp/allip -I %s"
...
def prepare_iptunnel_doc(context, mode):
    ...
    assert (
        context.command_code("ping -c 1 198.51.100.5") == 0
    ), "unable to ping public IP of B from A"
    assert (
        context.command_code("ip netns exec iptunnelB ping -c 1 203.0.113.10") == 0
    ), "unable to ping public IP of A from B"

    # preapre Network B part of iptunnel (in iptunnelB namespace)
    context.command_code(
        "ip -n iptunnelB link add name tunB type %s local 198.51.100.5 remote 203.0.113.10"
        % (mode)
    )
