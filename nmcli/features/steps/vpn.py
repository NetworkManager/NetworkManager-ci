...
@step(
    u'Use certificate "{cert}" with key "{key}" and authority "{ca}" for gateway "{gateway}" on OpenVPN connection "{name}"'
)
...
@step(
    u'Use user "{user}" with password "{password}" and group "{group}" with secret "{secret}" for gateway "{gateway}" on Libreswan connection "{name}"'
)
def set_libreswan_connection(context, user, password, group, secret, gateway, name):
    if (
        nmci.command_code("rpm -qa | grep -q libreswan-4") == 0
...
@step(
    u'Use user "{user}" with secret "{secret}" for gateway "{gateway}" on Strongswan connection "{name}"'
)
...
@step(
    u'Use user "{user}" with password "{password}" and group "{group}" with secret "{secret}" for gateway "{gateway}" on VPNC connection "{name}"'
)
...
@step(
    u'Use user "{user}" with password "{password}" and MPPE set to "{mppe}" for gateway "{gateway}" on PPTP connection "{name}"'
)
