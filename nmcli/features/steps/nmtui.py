...
def go_until_pattern_matches_aftercursor_text(
    ...
    for i in range(0, limit):
        match = re.match(
            pattern,
            context.screen.display[context.screen.cursor.y][
                context.screen.cursor.x + pre_c :
            ],
            re.UNICODE,
        )
...
def start_nmtui(context):
    context.tui = pexpect.spawn(
        'sh -c "TERM=%s nmtui > %s"' % (TERM_TYPE, OUTPUT),
        encoding="utf-8",
        codec_errors="ignore",
    )
...
def select_con_in_list(context, con_name):
    match = re.match(
        ".*Delete.*", get_screen_string(context.screen), re.UNICODE | re.DOTALL
    )
    if match is not None:
...
def set_specific_field_to(context, field, value):
    ...
    assert (
        go_until_pattern_matches_line(
            context, keys["DOWNARROW"], "^[\u2500-\u2599\\s]+%s.*" % field
        )
        is not None
...
def add_in_property(context, prop, value):
    assert (
        go_until_pattern_matches_line(
            context, keys["DOWNARROW"], "^.*[\u2502\\s]+%s <Add.*" % prop
        )
        is not None
...
def add_route(context, values):
    assert (
        go_until_pattern_matches_line(
            context, keys["DOWNARROW"], "^[\u2502\\s]+Routing.+<Edit"
        )
        is not None
...
def remove_items(context, prop):
    assert (
        go_until_pattern_matches_line(
            context, keys["DOWNARROW"], "^.*[\u2502\\s]+%s.*" % prop
        )
        is not None
    ), ("Could not find '%s' property!" % prop)
    while (
...
def come_in_category(context, category):
    assert (
        go_until_pattern_matches_line(
            context, keys["DOWNARROW"], "^.*[\u2550|\u2564]\\s%s.*" % category
        )
        is not None
...
def ensure_toggle_is_checked(context, toggle, n=None):
    match = go_until_pattern_matches_line(
        context, keys["DOWNARROW"], "^[\u2500-\u2599\\s]+(\[.\])\\s+%s.*" % toggle
    )
