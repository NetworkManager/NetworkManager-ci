...
def add_secondary_addr_same_subnet(context, device):
    ...
    assert (
        context.command_code(
            "ip addr add dev %s %s/%d"
            % (device, str(secondary_ip), primary_ipn.prefixlen)
        )
        == 0
    )
...
@step('Create device "{dev}" in "{ns}" with address "{addr}"')
def create_device_in_ns(context, dev, ns, addr):
    context.command_code(
        "ip -n %s link add %s type veth peer name %sp" % (ns, dev, dev)
    )
...
@step('Note the "{prop}" property from ifconfig output for device "{device}"')
...
@step('Rename device "{old_device}" to "{new_device}"')
...
def flag_cap_set(context, flag, n=None, device="wlan0", giveexception=True):
    ...
    ret = int(
        subprocess.check_output(cmd, shell=True).decode("utf-8", "ignore").strip()
    )
    ...
    if n is None:
        ...
        assert wcaps[flag] & ret != wcaps[flag], (
            "The flag is set! WirelessCapabilities: %d" % ret
        )
