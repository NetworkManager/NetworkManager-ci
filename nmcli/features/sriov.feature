 Feature: nmcli: sriov
    ...
    * Prepare "98-sriov.conf" config for "p6p2" device with "63" VFs
    ...
    When "1" is visible with command "nmcli dev |grep p6p1  |wc -l"
    * Prepare "99-sriov.conf" config for "p6p1" device with "2" VFs
    When "3" is visible with command "nmcli dev |grep p6p1  |wc -l"
    * Prepare "99-sriov.conf" config for "p6p1" device with "0" VFs
    When "1" is visible with command "nmcli dev |grep p6p1  |wc -l"
    ...
    When "1" is visible with command "nmcli dev |grep p6p1  |wc -l"
    When "1" is visible with command "nmcli dev |grep p6p2  |wc -l"
    * Prepare "99-sriov.conf" config for "p6p2" device with "2" VFs
    * Add a new connection of type "ethernet" and options "ifname p6p2_0 con-name ethie autoconnect no ipv4.method manual ipv4.address 1.2.3.4/24"
    * Add a new connection of type "ethernet" and options "ifname p6p2 con-name connie autoconnect no ipv4.method manual ipv4.address 1.2.3.5/24"
    ...
    * Bring "up" connection "connie"
    ...
    When "1" is visible with command "nmcli dev |grep p6p2  |wc -l"
    * Prepare "99-sriov.conf" config for "p6p2" device with "2" VFs
    * Add a new connection of type "ethernet" and options "ifname p6p2_0 con-name ethie ipv4.method manual ipv4.address 1.2.3.4/24"
    * Add a new connection of type "ethernet" and options "ifname p6p2 con-name connie ipv4.method manual ipv4.address 1.2.3.5/24"
    Then "activated" is visible with command "nmcli -g GENERAL.STATE con show ethie" in "5" seconds
     And "activated" is visible with command "nmcli -g GENERAL.STATE con show connie" in "5" seconds
    ...
    When "1" is visible with command "nmcli dev |grep p6p1  |wc -l"
    When "1" is visible with command "nmcli dev |grep p6p2  |wc -l"
    * Execute "echo 2 > /sys/class/net/p6p1/device/sriov_numvfs"
    * Execute "ip add add 192.168.1.2/24 dev p6p1_0"
    Then "192.168.1.2/24" is visible with command "nmcli con sh p6p1_0 |grep IP4" in "2" seconds
