...
RUN PKGS="systemd iproute NetworkManager ./*.rpm openvpn NetworkManager-openvpn sssd-client \
    ...
    if ls ./*.rpm | grep -q 'NetworkManager'; then \
        PKGS=$(echo "$PKGS" | sed 's/NetworkManager/dnsconfd*/')" */usr/bin/dig"; \
    fi && \
