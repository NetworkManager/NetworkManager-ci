...
class MockCloudMDRequestHandler(BaseHTTPRequestHandler):
    ...
    def _path_to_provider(self):
        """Returns: the provider, or None in case of error.
        In case of error, this function already emits a 404, the caller must return
        """
        ...
        if provider is None:
            self._response_and_end(404, write=f"no provider matches for {self.path}")
            return None
        elif provider not in self.server.config_get_providers():
            self._response_and_end(404, write=f"{provider} provider is disabled")
            return None
    ...
    def do_GET(self):
        ...
        provider = self._path_to_provider()
        if not provider:
            return
    ...
    def do_PUT(self):
        ...
        if provider := self._path_to_provider():
            try:
                content = self._read()
