...
def embed_file_if_exists(
    ...
    if os.path.isfile(fname):
        ...
        if remove:
            os.remove(fname)
        if mime_type == "link":
            embed_els = context.html_formatter.actual[
                "act_step_embed_span"
            ].getchildren()
            if len(embed_els) < 2:
                return
...
def embed_dump(context, dump_id, dump_output, caption, do_report):
    ...
    if not context.crashed_step:
        if context.nm_restarted:
            ...
            if do_report:
                context.crashed_step = (
                    "crash outside steps (envsetup, before / after scenario...)"
                )
...
def check_faf(context, do_report=True):
    ...
    for dump_dir in list_of_dumps:
        if not dump_dir:
            continue
        print("Examing crash: " + dump_dir)
        if not os.path.isfile("%s/pkg_name" % (dump_dir)) or not os.path.isfile(
            "%s/last_occurrence" % (dump_dir)
        ):
...
def setup_hostapd_wireless(context):
    ...
    if (
        context.command_code("sh prepare/hostapd_wireless.sh tmp/8021x/certs namespace")
        != 0
    ):
...
def restart_NM_service(context):
    ...
    context.run(
        "systemctl reset-failed NetworkManager.service ; systemctl restart NetworkManager.service"
    )
