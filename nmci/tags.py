...
def wireless_certs_bs(context, scenario):
    ...
    if not os.path.isfile("/tmp/certs/eaptest_ca_cert.pem"):
        context.process.run_stdout(
            "wget --tries=5 --retry-connrefused --retry-on-http-error=404,500,502 --waitretry=2 http://hpe-dl380pgen9-02.wlan.rhts.eng.bos.redhat.com/ca.pem -q -O /tmp/certs/eaptest_ca_cert.pem"
        )
    if not os.path.isfile("/tmp/certs/client.pem"):
        context.process.run_stdout(
            "wget --tries=5 --retry-connrefused --retry-on-http-error=404,500,502 --waitretry=2 http://hpe-dl380pgen9-02.wlan.rhts.eng.bos.redhat.com/client.pem -q -O /tmp/certs/client.pem"
        )
...
def tcpdump_bs(context, scenario):
    ...
    context.pexpect_service(
        "stdbuf -oL -eL tcpdump -nne -i any >> /tmp/network-traffic.log", shell=True
    )
