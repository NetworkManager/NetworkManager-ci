...
def tag1000_bs(ctx, scen):
    nmci.lib.wait_for_testeth0(ctx)
    if ctx.command_code("python -m pip install pyroute2") != 0:
...
def gsm_bs(ctx, scen):
    ...
    if not os.path.isfile("/tmp/usb_hub"):
        ...
        while True:
            print(
                "* looking for gsm lock in nfs nest.test.redhat.com:/mnt/qa/desktop/broadband_lock"
            )
...
def gsm_as(ctx, scen):
    ...
    if not os.path.isfile("/tmp/usb_hub"):
        ctx.run(
            "mount -o remount -t nfs nest.test.redhat.com:/mnt/qa/desktop/broadband_lock /mnt/scratch"
        )
...
def ethernet_as(ctx, scen):
    ...
    if "ipv4" not in scen.tags and "ipv6" not in scen.tags:
        ...
        ctx.run(
            "sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-ethernet*"
...
def attach_hostapd_log_as(ctx, scen):
    ...
    ctx.run(
        "sudo journalctl -u nm-hostapd --no-pager -o cat %s >> /tmp/journal-hostapd.log"
        % ctx.log_cursor
    )
...
def attach_wpa_supplicant_log_as(ctx, scen):
    ...
    ctx.run(
        "journalctl -u wpa_supplicant --no-pager -o cat %s >> /tmp/journal-wpa_supplicant.log"
        % ctx.log_cursor
    )
...
def preserve_8021x_certs_bs(ctx, scen):
    ...
    assert (
        ctx.command_code("cp -r tmp/8021x/certs/client/* /tmp/certs/") == 0
...
def openvswitch_as(ctx, scen):
    ...
    ctx.run(
        "sudo nmcli con del eth1 eth2 ovs-bond0 ovs-port0 ovs-patch0 ovs-patch1 ovs-bridge1 ovs-bridge0 ovs-port1 ovs-eth2 ovs-eth3 ovs-iface0 eth2 dpdk-sriov c-ovs-br0 c-ovs-port0 c-ovs-iface0 ovs-testX"
    )  # to be sure
    time.sleep(1)
...
def pppoe_bs(ctx, scen):
    ...
    ctx.run(
        "rpm -q NetworkManager-ppp || yum -y install NetworkManager-ppp -x NetworkManager"
    )
    ctx.run("rpm -q rp-pppoe || yum -y install rp-pppoe")
    ctx.run(
        "[ -x //usr/sbin/pppoe-server ] || yum -y install https://kojipkgs.fedoraproject.org//packages/rp-pppoe/3.12/11.fc28/$(uname -p)/rp-pppoe-3.12-11.fc28.$(uname -p).rpm"
    )
...
def tcpdump_bs(ctx, scen):
    with open("/tmp/network-traffic.log", "w") as f:
        f.write("~~~~~~~~~~~~~~~~~~~~~~~~~~ TRAFFIC LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    ctx.pexpect_service(
        "/bin/bash", ["-c", "sudo tcpdump -nne -i any >> /tmp/network-traffic.log"]
    )
...
def bridge_as(ctx, scen):
    if ctx.IS_NMTUI:
        ...
        time.sleep(1)
        ctx.run(
            "sudo nmcli connection delete id bridge0 bridge-slave-eth1 bridge-slave-eth2"
        )
        time.sleep(1)
        ...
        ctx.run(
            "sudo nmcli con del bridge4 bridge4.0 bridge4.1 nm-bridge eth4.80 eth4.90"
        )
        ctx.run(
            "sudo nmcli con del bridge-slave-eth4 bridge-nonslave-eth4 bridge-slave-eth4.80 eth4"
        )
        ctx.run(
            "sudo nmcli con del bridge0 bridge bridge.15 nm-bridge br88 br11 br12 br15 bridge-slave br15-slave br15-slave1 br15-slave2 br10 br10-slave"
        )
...
def vlan_as(ctx, scen):
    if ctx.IS_NMTUI:
        ...
        ctx.run(
            "sudo nmcli con del vlan vlan1 vlan2 eth7.99 eth7.99 eth7.299 eth7.399 eth7.65 eth7.165 eth7.265 eth7.499 eth7.80 eth7.90"
        )
        ctx.run(
            "sudo nmcli con del vlan_bridge7.15 vlan_bridge7 vlan_vlan7 vlan_bond7 vlan_bond7.7 vlan_team7 vlan_team7.1 vlan_team7.0"
        )
...
def team_as(ctx, scen):
    if ctx.IS_NMTUI:
        ...
        # sleep(TIMER)
        ctx.run(
            "if nmcli con |grep 'team0 '; then echo 'team0 present: %s' >> /tmp/residues; fi"
            % scen.tags
        )
...
def team_slaves_as(ctx, scen):
    ctx.run(
        "nmcli connection delete id team0.0 team0.1 team-slave-eth5 team-slave-eth6 eth5 eth6 team-slave"
    )
...
def allow_veth_connections_bs(ctx, scen):
    if (
        ctx.command_code(
            'grep \'^ENV{ID_NET_DRIVER}=="veth", ENV{NM_UNMANAGED}="1"\' /usr/lib/udev/rules.d/85-nm-unmanaged.rules'
        )
        == 0
    ):
        ctx.run(
            'sed -i \'s/^ENV{ID_NET_DRIVER}=="veth", ENV{NM_UNMANAGED}="1"/#ENV{ID_NET_DRIVER}=="veth", ENV{NM_UNMANAGED}="1"/\' /usr/lib/udev/rules.d/85-nm-unmanaged.rules'
        )
        ctx.run("udevadm control --reload-rules")
        ctx.run("udevadm settle --timeout=5")
...
def con_ipv4_remove_as(ctx, scen):
    ...
    ctx.run(
        "if nmcli con |grep con_ipv4; then echo 'con_ipv4 present: %s' >> /tmp/residues; fi"
        % scen.tags
    )
...
def con_ipv6_remove_as(ctx, scen):
    ...
    ctx.run(
        "if nmcli con |grep con_ipv6; then echo 'con_ipv6 present: %s' >> /tmp/residues; fi"
        % scen.tags
    )
...
def con_PBR_remove_as(ctx, scen):
    ctx.run(
        "sudo nmcli connection delete id Servers Internal-Workstations Provider-A Provider-B"
    )
...
def slaves_as(ctx, scen):
    ...
    nmci.lib.reset_hwaddr_nmcli(ctx, "eth4")
    ctx.run(
        "nmcli connection delete id bond0.0 bond0.1 bond0.2 bond-slave-eth1 bond-slave"
    )
