...
def skip_in_centos_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q -e 'CentOS Linux release 8' /etc/redhat-release") == 0:
        sys.exit(77)
...
def not_on_aarch64_but_pegas_bs(ctx, scen):
    ver = nmci.run.command_output("uname -r").strip()
    if ctx.arch == "aarch64":
        if "4.5" in ver:
            sys.exit(77)
...
def gsm_sim_bs(ctx, scen):
    if ctx.arch != "x86_64":
        ...
        sys.exit(77)
    nmci.run.run("sudo prepare/gsm_sim.sh modemu", stdout=None, stderr=None)


def gsm_sim_as(ctx, scen):
    nmci.run.run("nmcli con down id gsm")
    time.sleep(2)
    nmci.run.run("sudo prepare/gsm_sim.sh teardown")
    time.sleep(1)
    nmci.run.run("nmcli con del id gsm")
...
def not_with_systemd_resolved_bs(ctx, scen):
    print("---------------------------")
    if nmci.run.run("systemctl is-active systemd-resolved") == 0:
        sys.exit(77)
...
def not_under_internal_DHCP_bs(ctx, scen):
    if nmci.run.command_code("grep -q Ootpa /etc/redhat-release") == 0 and \
       nmci.run.command_code("NetworkManager --print-config|grep dhclient") != 0:
        sys.exit(77)
    if nmci.run.command_code("NetworkManager --print-config|grep internal") == 0:
        sys.exit(77)
...
def regenerate_veth_as(ctx, scen):
    ...
    if os.path.isfile('/tmp/nm_newveth_configured'):
        nmci.run.run('sh prepare/vethsetup.sh check')
    else:
        for link in range(1,11):
            nmci.run.run('ip link set eth%d up' % link)
...
def restart_as(ctx, scen):
    ...
    if nmci.run.command_code("systemctl is-active NetworkManager") != 0:
        nmci.run.run('sudo systemctl restart NetworkManager')
...
def secret_key_reset_bs(ctx, scen):
    nmci.run.run("mv /var/lib/NetworkManager/secret_key /var/lib/NetworkManager/secret_key_back")


def secret_key_reset_as(ctx, scen):
    nmci.run.run("mv /var/lib/NetworkManager/secret_key_back /var/lib/NetworkManager/secret_key")
...
def tag1000_bs(ctx, scen):
    ...
    nmci.lib.wait_for_testeth0()
    if nmci.run.command_code("python -m pip install pyroute2") != 0:
        nmci.run.run("yum -y install http://dl.fedoraproject.org/pub/epel/7/x86_64/p/python2-pyroute2-0.4.13-1.el7.noarch.rpm")


def tag1000_as(ctx, scen):
    print("---------------------------")
    print("deleting bridge0 and 1000 dummy devices")
    nmci.run.run("ip link del bridge0")
    nmci.run.run("for i in $(seq 0 1000); do ip link del port$i ; done")
...
def captive_portal_bs(ctx, scen):
    ...
    nmci.run.run("sudo prepare/captive_portal.sh", stdout=None, stderr=None)


def captive_portal_as(ctx, scen):
    nmci.run.run("sudo prepare/captive_portal.sh teardown")
...
def gsm_bs(ctx, scen):
    nmci.run.run("mmcli -G debug")
    nmci.run.run("nmcli general logging level DEBUG domains ALL")
...
def gsm_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete gsm')
    nmci.run.run('rm -rf /etc/NetworkManager/system-connections/gsm')
    nmci.run.run('nmcli con up testeth0')
    nmci.lib.wait_for_testeth0()
    if not os.path.isfile('/tmp/usb_hub'):
        nmci.run.run('mount -o remount -t nfs nest.test.redhat.com:/mnt/qa/desktop/broadband_lock /mnt/scratch')
        nmci.lib.delete_old_lock("/mnt/scratch/", nmci.lib.get_lock("/mnt/scratch"))
    # Attach journalctl logs
    nmci.run.run("echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ MM LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-mm.log")
    print("Attaching MM log")
    nmci.run.run("sudo journalctl -u ModemManager --no-pager -o cat %s >> /tmp/journal-mm.log" % ctx.log_cursor)
...
def unmanage_eth_bs(ctx, scen):
    ...
    for link in links:
        nmci.run.run('nmcli dev set %s managed no' % link)


def unmanage_eth_as(ctx, scen):
    links = nmci.lib.get_ethernet_devices()
    for link in links:
        nmci.run.run('nmcli dev set %s managed yes' % link)
...
def manage_eth8_as(ctx, scen):
    ...
    nmci.run.run('sudo nmcli device set eth8 managed true')
...
def connectivity_bs(ctx, scen):
    ...
    nmci.run.run("echo '[connectivity]' > /etc/NetworkManager/conf.d/99-connectivity.conf")
    if 'captive_portal' in scen.tags:
        nmci.run.run("echo 'uri=http://static.redhat.com:8001/test/rhel-networkmanager.txt' >> /etc/NetworkManager/conf.d/99-connectivity.conf")
    else:
        nmci.run.run("echo 'uri=http://static.redhat.com/test/rhel-networkmanager.txt' >> /etc/NetworkManager/conf.d/99-connectivity.conf")
    nmci.run.run("echo 'response=OK' >> /etc/NetworkManager/conf.d/99-connectivity.conf")
    # Change in interval  would affect connectivity tests and captive portal tests too
    nmci.run.run("echo 'interval=10' >> /etc/NetworkManager/conf.d/99-connectivity.conf")
...
def connectivity_as(ctx, scen):
    ...
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/99-connectivity.conf")
    nmci.run.run("rm -rf /var/lib/NetworkManager/NetworkManager-intern.conf")
...
def unload_kernel_modules_bs(ctx, scen):
    nmci.run.run("modprobe -r qmi_wwan")
    nmci.run.run("modprobe -r cdc-mbim")
...
def disp_bs(ctx, scen):
    ...
    nmci.run.run("> /tmp/dispatcher.txt")


def disp_as(ctx, scen):
    print("---------------------------")
    print("deleting dispatcher files")
    nmci.run.run("rm -rf /etc/NetworkManager/dispatcher.d/*-disp")
    nmci.run.run("rm -rf /usr/lib/NetworkManager/dispatcher.d/*-disp")
    nmci.run.run("rm -rf /etc/NetworkManager/dispatcher.d/pre-up.d/98-disp")
    nmci.run.run("rm -rf /etc/NetworkManager/dispatcher.d/pre-down.d/97-disp")
    #nmci.run.run("rm -rf /tmp/dispatcher.txt")
    nmci.run.run('nmcli con down testeth1')
    nmci.run.run('nmcli con down testeth2')
    nmci.lib.reload_NM_service()
...
def eth0_bs(ctx, scen):
    ...
    #    print("---------------------------")
    #    print("eth0")
    #    nmci.run.run("nmcli connection down id testeth0")
    #    time.sleep(1)
    #    if nmci.run.command_code("nmcli -f NAME c sh -a |grep eth0") == 0:
    #        print("shutting down eth0 once more as it is not down")
    #        nmci.run.run("nmcli device disconnect eth0")
    #        time.sleep(2)
    #    print("---------------------------")
    print("---------------------------")
    print("eth0 disconnect")
    nmci.run.run("nmcli con down testeth0")
    nmci.run.run('nmcli con down testeth1')
    nmci.run.run('nmcli con down testeth2')
...
#            nmci.run.run('hostnamectl set-hostname --transien ""')
#            nmci.run.run('hostnamectl set-hostname --static %s' % ctx.original_hostname)
    nmci.lib.wait_for_testeth0()
...
def alias_bs(ctx, scen):
    ...
    nmci.run.run("nmcli connection up testeth7")
    nmci.run.run("nmcli connection delete eth7")
...
def alias_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete eth7")
    nmci.run.run("sudo rm -f /etc/sysconfig/network-scripts/ifcfg-eth7:0")
    nmci.run.run("sudo rm -f /etc/sysconfig/network-scripts/ifcfg-eth7:1")
    nmci.run.run("sudo rm -f /etc/sysconfig/network-scripts/ifcfg-eth7:2")
    nmci.run.run("sudo nmcli connection reload")
    nmci.run.run("nmcli connection down testeth7")
    #nmci.run.run('sudo nmcli con add type ethernet ifname eth7 con-name testeth7 autoconnect no')
    #sleep(TIMER)
...
def netcat_bs(ctx, scen):
    ...
    if not os.path.isfile('/usr/bin/nc'):
        nmci.run.run('sudo yum -y install nmap-ncat')
...
def scapy_bs(ctx, scen):
    ...
    if not os.path.isfile('/usr/bin/scapy'):
        nmci.run.run('yum -y install tcpdump')
        nmci.run.run("python -m pip install scapy")


def scapy_as(ctx, scen):
    print("---------------------------")
    print("removing veth devices")
    nmci.run.run("ip link delete test10")
    nmci.run.run("ip link delete test11")
    nmci.run.run("nmcli connection delete ethernet-test10 ethernet-test11")
...
def mock_bs(ctx, scen):
    ...
    if nmci.run.command_code('rpm -q --quiet dbus-x11') != 0:
        nmci.run.run('yum -y install dbus-x11')
    if nmci.run.command_code('python -m pip list |grep python-dbusmock') != 0:
        nmci.run.run("sudo python -m pip install python-dbusmock")
    nmci.run.run('./tmp/patch-python-dbusmock.sh')
...
def IPy_bs(ctx, scen):
    ...
    if nmci.run.command_code('rpm -q --quiet dbus-x11') != 0:
        nmci.run.run('yum -y install dbus-x11')
    if nmci.run.command_code('python -m pip list |grep IPy') != 0:
        nmci.run.run("sudo python -m pip install IPy")
...
def netaddr_bs(ctx, scen):
    ...
    nmci.lib.wait_for_testeth0()
    if nmci.run.command_code('python -m pip list |grep netaddr') != 0:
        nmci.run.run("sudo python -m pip install netaddr")
...
def inf_bs(ctx, scen):
    ...
    nmci.run.run("nmcli device disconnect inf_ib0")
    nmci.run.run("nmcli device disconnect inf_ib0.8002")
    nmci.run.run("nmcli connection delete inf_ib0.8002")
    nmci.run.run("nmcli connection delete id infiniband-inf_ib0.8002 inf.8002 inf inf2 infiniband-inf_ib0 infiniband")


def inf_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id infiniband0 infiniband0-port")
    else:
        print("---------------------------")
        print("deleting infiniband connections")
        nmci.run.run("nmcli connection up id tg3_1")
        nmci.run.run("nmcli connection delete id inf inf2 infiniband inf.8002")
        nmci.run.run("nmcli device connect inf_ib0.8002")
...
def dsl_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id dsl0")
...
def dns_dnsmasq_bs(ctx, scen):
    ...
    if nmci.run.command_code("systemctl is-active systemd-resolved") == 0:
        ctx.systemd_resolved = True
        nmci.run.run("systemctl stop systemd-resolved")
        nmci.run.run("rm -rf /etc/resolv.conf")
    else:
        ctx.systemd_resolved = False
    nmci.run.run("printf '# configured by beaker-test\n[main]\ndns=dnsmasq\n' > /etc/NetworkManager/conf.d/99-xtest-dns.conf")
    nmci.lib.reload_NM_service()
...
def dns_dnsmasq_as(ctx, scen):
    ...
    nmci.run.run("rm -f /etc/NetworkManager/conf.d/99-xtest-dns.conf")
    nmci.lib.reload_NM_service()
    ctx.dns_plugin = ""
...
def dns_systemd_resolved_bs(ctx, scen):
    ...
    if nmci.run.command_code("systemctl is-active systemd-resolved") != 0:
        ...
        nmci.run.run("timeout 60 systemctl start systemd-resolved")
        if nmci.run.command_code("systemctl is-active systemd-resolved") != 0:
...
def internal_DHCP_as(ctx, scen):
    ...
    nmci.run.run("rm -f /etc/NetworkManager/conf.d/99-xtest-dhcp-internal.conf")
    nmci.lib.restart_NM_service()
...
def dhclient_DHCP_as(ctx, scen):
    ...
    nmci.run.run("rm -f /etc/NetworkManager/conf.d/99-xtest-dhcp-dhclient.conf")
    nmci.lib.restart_NM_service()
...
def dummy_bs(ctx, scen):
    ...
    nmci.run.run("ip link add dummy0 type dummy")
    nmci.run.run("ip link delete dummy0")


def dummy_as(ctx, scen):
    print("---------------------------")
    print("removing dummy and bridge/bond/team devices")
    nmci.run.run("nmcli con del dummy0 dummy1")
    nmci.run.run("ip link delete dummy0")
    nmci.run.run("ip link del br0")
    nmci.run.run("ip link del vlan")
    nmci.run.run("ip link del bond0")
    nmci.run.run("ip link del team0")
...
def delete_testeth0_bs(ctx, scen):
    ...
    nmci.run.run("nmcli device disconnect eth0")
    nmci.run.run("nmcli connection delete id testeth0")


def delete_testeth0_as(ctx, scen):
    print("---------------------------")
    print("restoring testeth0 profile")
    nmci.run.run('sudo nmcli connection delete eth0')
...
def ifcfg_rh_bs(ctx, scen):
    if nmci.run.command_code("NetworkManager --print-config |grep '^plugins=ifcfg-rh'") != 0:
        print("---------------------------")
...
def ifcfg_rh_as(ctx, scen):
    if nmci.run.run('test -f /etc/NetworkManager/conf.d/99-xxcustom.conf') == 0:
        print("---------------------------")
        print("resetting ifcfg plugin")
        nmci.run.run('sudo rm -f /etc/NetworkManager/conf.d/99-xxcustom.conf')
        nmci.lib.restart_NM_service()
        if ctx.IS_NMTUI:
            # if 'simwifi' in scen.tags:
            #     nmci.lib.wifi_rescan()
...
def eth3_disconnect_bs(ctx, scen):
    ...
    nmci.run.run('sudo nmcli device disconnect eth3')
    nmci.run.run('sudo kill -9 $(cat /var/run/dhclient-eth3.pid)')
...
def eth3_disconnect_as(ctx, scen):
    ...
    nmci.run.run('sudo nmcli connection up testeth3')
    nmci.run.run('sudo nmcli connection down testeth3')
...
def need_dispatcher_scripts_bs(ctx, scen):
    ...
    if os.path.isfile("/tmp/nm-builddir"):
        nmci.run.run('yum install -y $(cat /tmp/nm-builddir)/noarch/NetworkManager-dispatcher-routing-rules*')
    else:
        nmci.lib.wait_for_testeth0()
        nmci.run.run("yum -y install NetworkManager-config-routing-rules")
    nmci.lib.reload_NM_service()
...
def need_dispatcher_scripts_as(ctx, scen):
    ...
    nmci.lib.wait_for_testeth0()
    nmci.run.run("yum -y remove NetworkManager-config-routing-rules ")
    nmci.run.run("rm -rf /etc/sysconfig/network-scripts/rule-con_general")
    nmci.run.run('rm -rf /etc/sysconfig/network-scripts/route-con_general')
    nmci.run.run('ip rule del table 1; ip rule del table 1')
    nmci.lib.reload_NM_service()
...
def ethernet_bs(ctx, scen):
    ...
    if nmci.run.command_code('nmcli con |grep testeth1') == 0 or nmci.run.run('nmcli con |grep testeth2') == 0:
        nmci.run.run('sudo nmcli con del testeth1 testeth2')
        nmci.run.run('sudo nmcli con add type ethernet ifname eth1 con-name testeth1 autoconnect no')
        nmci.run.run('sudo nmcli con add type ethernet ifname eth2 con-name testeth2 autoconnect no')


def ethernet_as(ctx, scen):
    nmci.run.run("sudo nmcli connection delete id ethernet ethernet1 ethernet2")

    if 'ipv4' not in scen.tags and 'ipv6' not in scen.tags:
        print("---------------------------")
        print("removing ethernet profiles")
        nmci.run.run("sudo nmcli connection delete id ethernet ethernet0 ethos")
        nmci.run.run('sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-ethernet*') #ideally should do nothing
...
def logging_bs(ctx, scen):
    ctx.loggin_level = nmci.run.command_output('nmcli -t -f LEVEL general logging').strip()


def logging_as(ctx, scen):
    print("---------------------------")
    print("setting log level back")
    nmci.run.run('sudo nmcli g log level %s domains ALL' % ctx.loggin_level)
...
def logging_info_only_bs(ctx, scen):
    ...
    nmci.run.run("echo '[logging]' > %s" % log)
    nmci.run.run("echo 'level=INFO' >> %s" % log)
    nmci.run.run("echo 'domains=ALL' >> %s" % log)
    time.sleep(0.5)
    nmci.lib.restart_NM_service()
    time.sleep(1)
...
def netservice_bs(ctx, scen):
    ...
    nmci.run.run("sudo pkill -9 /sbin/dhclient")
    # Make orig- devices unmanaged as they may be unfunctional
    nmci.run.run("for dev in $(nmcli  -g DEVICE d |grep orig); do nmcli device set $dev managed off; done")
    nmci.lib.restart_NM_service()
    nmci.run.run("sudo systemctl restart network.service")
    nmci.run.run("nmcli connection up testeth0")
    time.sleep(1)
...
def netservice_as(ctx, scen):
    ...
    nmci.run.run("echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ NETWORK SRV LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-netsrv.log")
    nmci.run.run("sudo journalctl -u network --no-pager -o cat %s >> /tmp/journal-netsrv.log" % ctx.log_cursor)
...
def tag8021x_bs(ctx, scen):
    print("---------------------------")
    if ctx.arch == "s390x":
        nmci.run.run("[ -x /usr/sbin/hostapd ] || (yum -y install 'https://vbenes.fedorapeople.org/NM/hostapd-2.6-7.el7.s390x.rpm'; time.sleep 10)")
...
def simwifi_as(ctx, scen):
    if ctx.IS_NMTUI:
        ...
        nmci.run.run("nmcli con del uuid $(nmcli -t -f uuid,type con show | grep ':802-11-wireless$' | sed 's/:802-11-wireless$//g' )")
...
def simwifi_ap_bs(ctx, scen):
    ...
    if ctx.arch != "x86_64":
        sys.exit(77)
    nmci.run.run("modprobe -r mac80211_hwsim")
    nmci.run.run("modprobe mac80211_hwsim")
    nmci.run.run("systemctl restart wpa_supplicant")
    nmci.run.run("systemctl restart NetworkManager")


def simwifi_ap_as(ctx, scen):
    print("---------------------------")
    print("deleting wifi AP connections")
    nmci.run.run("nmcli con del wifi-ap wifi-client br0 br0-slave1 br0-slave2")
    nmci.run.run("ip link delete br0")
...
def simwifi_p2p_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q 'release 8' /etc/redhat-release") == 0:
        nmci.run.run("dnf -4 -y install "
    ...
    if nmci.run.command_code("ls /tmp/nm_*_supp_configured") == 0:
...
def simwifi_p2p_as(ctx, scen):
    print("---------------------------")
    if nmci.run.command_code("grep -q 'release 8' /etc/redhat-release") == 0:
        nmci.run.run("dnf -4 -y install https://vbenes.fedorapeople.org/NM/rhbz1888051/wpa_supplicant{,-debuginfo,-debugsource}-2.9-3.el8.$(arch).rpm")
        nmci.run.run("dnf -y update wpa_supplicant")
        nmci.run.run("systemctl restart wpa_supplicant")
    nmci.run.run('modprobe -r mac80211_hwsim')
    nmci.run.run('nmcli con del wifi-p2p')
    nmci.run.run("kill -9 $(ps aux|grep wpa_suppli |grep wlan1 |awk '{print $2}')")
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/99-wifi.conf")
...
def simwifi_wpa2_as(ctx, scen):
    ...
    nmci.run.run("nmcli con del wpa2-eap wifi")
...
def simwifi_wpa3_as(ctx, scen):
    ...
    print("deleting wifi connections")
    #teardown_hostapd_wireless()
    nmci.run.run("nmcli con del wpa3 wifi")
...
def simwifi_open_as(ctx, scen):
    print("---------------------------")
    print("deleting wifi connections")
    nmci.run.run("nmcli con del open")
...
def simwifi_pskwep_as(ctx, scen):
    print("---------------------------")
    print("deleting wifi connections")
    nmci.run.run("nmcli con del wep")
...
def simwifi_dynwep_as(ctx, scen):
    print("---------------------------")
    print("deleting wifi connections")
    nmci.run.run("nmcli con del wifi")
...
def vpnc_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q Maipo /etc/redhat-release") == 0:
        nmci.run.run("[ -f /etc/yum.repos.d/epel.repo ] || sudo rpm -i http://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm")
    if nmci.run.command_code("rpm -q NetworkManager-vpnc") != 0:
        nmci.run.run("sudo yum -y install NetworkManager-vpnc")
        nmci.lib.restart_NM_service()
...
def vpnc_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete vpnc')
...
def tcpreplay_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q Maipo /etc/redhat-release") == 0:
        nmci.run.run("[ -f /etc/yum.repos.d/epel.repo ] || sudo rpm -i http://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm")
    nmci.run.run("[ -x /usr/bin/tcpreplay ] || yum -y install tcpreplay")
...
def openvpn_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q Maipo /etc/redhat-release") == 0:
        nmci.run.run("[ -f /etc/yum.repos.d/epel.repo ] || sudo rpm -i http://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm")
    nmci.run.run("[ -x /usr/sbin/openvpn ] || sudo yum -y install openvpn NetworkManager-openvpn")
    if nmci.run.command_code("rpm -q NetworkManager-openvpn") != 0:
        nmci.run.run("sudo yum -y install NetworkManager-openvpn-1.0.8-1.el7.$(uname -p).rpm")
        nmci.lib.restart_NM_service()

    # This is an internal RH workaround for secondary architecures that are not present in EPEL

    nmci.run.run("[ -x /usr/sbin/openvpn ] || sudo yum -y install https://vbenes.fedorapeople.org/NM/openvpn-2.3.8-1.el7.$(uname -p).rpm\
                                                          https://vbenes.fedorapeople.org/NM/pkcs11-helper-1.11-3.el7.$(uname -p).rpm")
    nmci.run.run("rpm -q NetworkManager-openvpn || sudo yum -y install https://vbenes.fedorapeople.org/NM/NetworkManager-openvpn-1.0.8-1.el7.$(uname -p).rpm")
    nmci.lib.reload_NM_service()
    ...
    ctx.ovpn_proc = nmci.run.Popen("sudo openvpn /etc/openvpn/trest-server.conf",
...
def openvpn_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete openvpn')
    #nmci.run.run("sudo systemctl stop openvpn@test-server")
    print("teardown OpenVPN")
    nmci.run.run("sudo kill $(pidof openvpn)")
...
def libreswan_bs(ctx, scen):
    print("---------------------------")
    nmci.lib.wait_for_testeth0()
    if nmci.run.command_code("rpm -q NetworkManager-libreswan") != 0:
        nmci.run.run("sudo yum -y install NetworkManager-libreswan")
        nmci.lib.restart_NM_service()
    nmci.run.run("/usr/sbin/ipsec --checknss")
    mode = "aggressive"
...
def libreswan_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection down libreswan')
    nmci.run.run('nmcli connection delete libreswan')
...
def strongswan_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q 'release 7' /etc/redhat-release") == 0:
        if ctx.arch == "s390x":
...
def strongswan_as(ctx, scen):
    ...
    #nmci.run.run("ip route del default via 172.31.70.1")
    nmci.run.run('nmcli connection down strongswan')
    nmci.run.run('nmcli connection delete strongswan')
...
def vpn_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete vpn")
...
def iptunnel_bs(ctx, scen):
    ...
    nmci.run.run("modprobe -r xfrm_interface")
    nmci.run.run('sh prepare/iptunnel.sh')


def iptunnel_as(ctx, scen):
    print("----------------------------")
    print("iptunnel teardown")
    nmci.run.run('sh prepare/iptunnel.sh teardown')
...
def iptunnel_doc_as(ctx, scen):
    ...
    nmci.run.run("nmcli con delete gre1 tun0 bridge0 bridge0-port1 bridge0-port2")
    nmci.run.run("ip netns del iptunnelB")
    nmci.run.run("ip link del ipA")
    nmci.run.run("ip link del tunB")
    nmci.run.run("ip link del brB")
    nmci.run.run("ip link del bridge0")
...
def wireguard_bs(ctx, scen):
    ...
    rc = nmci.run.command_code('sh prepare/wireguard.sh')
    if rc != 0:
...
def wireguard_as(ctx, scen):
    ...
    nmci.run.run('nmcli con del wireguard')
...
def dracut_as(ctx, scen):
    ...
    if os.path.isfile("/tmp/dracut_setup.log"):
        ...
        nmci.run.run("rm -f /tmp/dracut_setup.log")
    if os.path.isfile("/tmp/dracut_clean.log"):
        ...
        nmci.run.run("rm -f /tmp/dracut_clean.log")
    nmci.run.run("journalctl -all --no-pager %s | grep ' dhcpd\\[' > /tmp/journal-dhcpd.log" % ctx.log_cursor)
    ctx.embed("text/plain", nmci.lib.utf_only_open_read("/tmp/journal-dhcpd.log"), "DHCPD")
    nmci.run.run("journalctl -all --no-pager %s | grep ' radvd\\[' > /tmp/journal-radvd.log" % ctx.log_cursor)
    ctx.embed("text/plain", nmci.lib.utf_only_open_read("/tmp/journal-radvd.log"), "RADVD")
    nmci.run.run("journalctl -all --no-pager %s | grep ' rpc.mountd\[' > /tmp/journal-nfs.log" % ctx.log_cursor)
    ctx.embed("text/plain", nmci.lib.utf_only_open_read("/tmp/journal-nfs.log"), "NFS")
    print("running after_test (free up dhcpd leases)")
    nmci.run.run("cd contrib/dracut; . ./setup.sh; after_test")
...
def prepare_patched_netdevsim_bs(ctx, scen):
    ...
    rc = nmci.run.command_code('sh prepare/netdevsim.sh setup',
...
def prepare_patched_netdevsim_as(ctx, scen):
    ...
    nmci.run.run('sh prepare/netdevsim.sh teardown')
...
def load_netdevsim_bs(ctx, scen):
    print("----------------------------")
    print("* prepare patched netdevsim setup")
    nmci.run.run('modprobe -r netdevsim; modprobe netdevsim')
    nmci.run.run("echo 1 1 > /sys/bus/netdevsim/new_device ; sleep 1")


def load_netdevsim_as(ctx, scen):
    print("----------------------------")
    print("* teardown patched netdevsim setup")
    nmci.run.run('modprobe -r netdevsim; sleep 1')
...
def attach_hostapd_log_as(ctx, scen):
    ...
    nmci.run.run("echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ HOSTAPD LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-hostapd.log")
    nmci.run.run("sudo journalctl -u nm-hostapd --no-pager -o cat %s >> /tmp/journal-hostapd.log" % ctx.log_cursor)
...
def attach_wpa_supplicant_log_as(ctx, scen):
    ...
    nmci.run.run("echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ WPA_SUPPLICANT LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-wpa_supplicant.log")
    nmci.run.run("journalctl -u wpa_supplicant --no-pager -o cat %s >> /tmp/journal-wpa_supplicant.log" % ctx.log_cursor)
...
def performance_bs(ctx, scen):
    ...
    if nmci.run.command_code("hostname |grep -q gsm-r5") != 0:
        print("** skipping")
        sys.exit(77)
    # NM needs to go down
...
def preserve_8021x_certs_bs(ctx, scen):
    print("---------------------------")
    nmci.run.run("curl -s https://raw.githubusercontent.com/NetworkManager/NetworkManager/master/libnm-core/tests/certs/test-key-and-cert.pem -o /tmp/test_key_and_cert.pem")
    nmci.run.run("curl -s https://raw.githubusercontent.com/NetworkManager/NetworkManager/master/libnm-core/tests/certs/test2_ca_cert.pem -o /tmp/test2_ca_cert.pem")
...
def pptp_bs(ctx, scen):
    ...
    if nmci.run.command_code("grep -q Maipo /etc/redhat-release") == 0:
        nmci.run.run("[ -f /etc/yum.repos.d/epel.repo ] || sudo rpm -i http://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm")
    nmci.run.run("[ -x /usr/sbin/pptpd ] || sudo yum -y install /usr/sbin/pptpd")
    nmci.run.run("rpm -q NetworkManager-pptp || sudo yum -y install NetworkManager-pptp")

    nmci.run.run("sudo rm -f /etc/ppp/ppp-secrets")
    ...
    if not os.path.isfile('/tmp/nm_pptp_configured'):
        ...
        nmci.run.run("sudo systemctl unmask pptpd")
        nmci.run.run("sudo systemctl restart pptpd")
        ...
        nmci.run.run("/sbin/pppd pty '/sbin/pptp 127.0.0.1' nodetach")
        #nmci.run.run("nmcli con up id pptp")
        #nmci.run.run("nmcli con del pptp")
        nmci.run.run("touch /tmp/nm_pptp_configured")
        time.sleep(1)
...
def pptp_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete pptp')
...
def firewall_bs(ctx, scen):
    ...
    if nmci.run.command_code("rpm -q firewalld") != 0:
        nmci.lib.wait_for_testeth0()
        nmci.run.run("sudo yum -y install firewalld")
    nmci.run.run("sudo systemctl unmask firewalld")
    time.sleep(1)
    nmci.run.run("sudo systemctl stop firewalld")
    time.sleep(5)
    nmci.run.run("sudo systemctl start firewalld")
    nmci.run.run("sudo nmcli con modify testeth0 connection.zone public")
...
def firewall_as(ctx, scen):
    ...
    nmci.run.run("sudo firewall-cmd --panic-off")
    nmci.run.run("sudo systemctl stop firewalld")
...
def restore_hostname_bs(ctx, scen):
    ...
    ctx.original_hostname = nmci.run.command_output('hostname').strip()


def restore_hostname_as(ctx, scen):
    print("---------------------------")
    print("restoring original hostname")
    nmci.run.run('systemctl unmask systemd-hostnamed.service')
    nmci.run.run('systemctl unmask dbus-org.freedesktop.hostname1.service')
    if ctx.IS_NMTUI:
        nmci.run.run('sudo echo "localhost.localdomain" > /etc/hostname')
    else:
        nmci.run.run('hostnamectl set-hostname --transien ""')
        nmci.run.run('hostnamectl set-hostname --static %s' % ctx.original_hostname)
    nmci.run.run('rm -rf /etc/NetworkManager/conf.d/90-hostname.conf')
    nmci.run.run('rm -rf /etc/dnsmasq.d/dnsmasq_custom.conf')
    nmci.lib.reload_NM_service()
    nmci.run.run("nmcli con up testeth0")
...
def runonce_bs(ctx, scen):
    ...
    nmci.run.run("systemctl stop network")
    nmci.run.run("nmcli device disconnect eth0")
    nmci.run.run("pkill -9 dhclient")
    nmci.run.run("pkill -9 nm-iface-helper")
    nmci.run.run("sudo systemctl stop firewalld")
...
def runonce_as(ctx, scen):
    ...
    nmci.run.run("for i in $(pidof nm-iface-helper); do kill -9 $i; done")
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/01-run-once.conf")
    time.sleep(1)
    nmci.lib.restart_NM_service()
    time.sleep(1)
    nmci.run.run("for i in $(pidof nm-iface-helper); do kill -9 $i; done")
    nmci.run.run("nmcli connection delete con_general")
    nmci.run.run("nmcli device disconnect eth10")
    nmci.run.run("nmcli connection up testeth0")
...
def slow_team_bs(ctx, scen):
    ...
    nmci.run.run("for i in $(rpm -qa |grep team|grep -v Netw); do rpm -e $i --nodeps; done")
    nmci.run.run("yum -y install https://vbenes.fedorapeople.org/NM/slow_libteam-1.25-5.el7_4.1.1.x86_64.rpm https://vbenes.fedorapeople.org/NM/slow_teamd-1.25-5.el7_4.1.1.x86_64.rpm")
    if nmci.run.command_code("rpm --quiet -q teamd") != 0:
        # Restore teamd package if we don't have the slow ones
        nmci.run.run("for i in $(rpm -qa |grep team|grep -v Netw); do rpm -e $i --nodeps; done")
        nmci.run.run("yum -y install teamd libteam")
        sys.exit(77)
    nmci.lib.reload_NM_service()
...
def openvswitch_bs(ctx, scen):
    ...
    if ctx.arch == "s390x" and nmci.run.run("grep -q Ootpa /etc/redhat-release") != 0:
        sys.exit(77)
    if nmci.run.command_code('rpm -q NetworkManager-ovs') != 0:
        nmci.run.run('yum -y install NetworkManager-ovs')
        nmci.run.run('systemctl daemon-reload')
        nmci.lib.restart_NM_service()
    if nmci.run.command_code('systemctl is-active openvswitch') != 0 or \
            nmci.run.command_code('systemctl status ovs-vswitchd.service |grep -q ERR') != 0:
        nmci.run.run('systemctl restart openvswitch')
        nmci.lib.restart_NM_service()
...
def openvswitch_as(ctx, scen):
    ...
    nmci.run.run("echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ OVSDB LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-mm.log")
    ...
    nmci.run.run("echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ OVSDemon LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-mm.log")
    ...
    nmci.run.run('sudo ifdown bond0')
    nmci.run.run('sudo ifdown eth1')
    nmci.run.run('sudo ifdown eth2')
    nmci.run.run('sudo ifdown ovsbridge0')
    nmci.run.run('sudo nmcli con del eth1 eth2 ovs-bond0 ovs-port0 ovs-patch0 ovs-patch1 ovs-bridge1 ovs-bridge0 ovs-port1 ovs-eth2 ovs-eth3 ovs-iface0 eth2 dpdk-sriov c-ovs-br0 c-ovs-port0 c-ovs-iface0') # to be sure
    time.sleep(1)
    nmci.run.run('ovs-vsctl del-br ovsbr0')
    nmci.run.run('ovs-vsctl del-br ovsbridge0')
    nmci.run.run('ovs-vsctl del-br ovsbridge1')
    nmci.run.run('ovs-vsctl del-br i-ovs-br0')
    nmci.run.run('nmcli device delete bond0')
    nmci.run.run('nmcli device delete port0')
    nmci.run.run('sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-eth1')
    nmci.run.run('sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-bond0')
    nmci.run.run('sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-ovsbridge0')
    nmci.run.run('sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-intbr0')
    nmci.run.run('sudo ip link set dev eth1 up')
    nmci.run.run('sudo ip link set dev eth2 up')
    nmci.run.run('sudo nmcli con reload')
    nmci.run.run('nmcli con up testeth1')
    nmci.run.run('nmcli con down testeth1')
    nmci.run.run('nmcli con up testeth2')
    nmci.run.run('nmcli con down testeth2')
...
def sriov_bs(ctx, scen):
    ...
    nmci.run.run('nmcli con del p4p1')
...
def sriov_as(ctx, scen):
    ...
    print("remove sriov")
    nmci.run.run("nmcli con del sriov")

    print("remove sriov_2")
    nmci.run.run("nmcli con del sriov_2")

    print("set 0 to /sys/class/net/*/device/sriov_numvfs")
    nmci.run.run("echo 0 > /sys/class/net/p6p1/device/sriov_numvfs")
    nmci.run.run("echo 0 > /sys/class/net/p4p1/device/sriov_numvfs")

    print("remove /etc/NetworkManager/conf.d/9*-sriov.conf")
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/99-sriov.conf")
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/98-sriov.conf")

    nmci.run.run("set 1 to /sys/class/net/p4p1/device/sriov_drivers_autoprobe")
    nmci.run.run("echo 1 > /sys/class/net/p4p1/device/sriov_drivers_autoprobe")
    nmci.run.run("echo 1 > /sys/class/net/p6p1/device/sriov_drivers_autoprobe")

    print("remove ixgbevf driver")
    nmci.run.run("modprobe -r ixgbevf")
...
def sriov_bond_as(ctx, scen):
    ...
    nmci.run.run("nmcli con del sriov2")
    nmci.run.run("nmcli con del sriov_bond0")
    nmci.run.run("nmcli con del sriov_bond0.0")
    nmci.run.run("nmcli con del sriov_bond0.1")
...
def dpdk_bs(ctx, scen):
    ...
    nmci.run.run("sysctl -w vm.nr_hugepages=10")

    print(" * install dpdk")
    nmci.run.run('yum -y install dpdk dpdk-tools')

    print(" add root:root to run hugetlbfs in /etc/openvswitch")
    nmci.run.run('sed -i.bak s/openvswitch:hugetlbfs/root:root/g /etc/sysconfig/openvswitch')

    print(" * enable dpdk in openvswitch")
    nmci.run.run('ovs-vsctl --no-wait set Open_vSwitch . other_config:dpdk-init=true')

    print(" * modprobe vfio-pci to be used as dpdk NIC driver")
    nmci.run.run('modprobe vfio-pci')

    print(" * enable unsafe_noiommu_mode")
    nmci.run.run('echo 1 > /sys/module/vfio/parameters/enable_unsafe_noiommu_mode')

    print(" * enable two VFs")
    nmci.run.run('nmcli  connection add type ethernet ifname p4p1 con-name dpdk-sriov sriov.total-vfs 2')
    nmci.run.run('nmcli  connection up dpdk-sriov')

    print(" * add both VFs to DPDK")
    nmci.run.run('dpdk-devbind -b vfio-pci 0000:42:10.0')
    nmci.run.run('dpdk-devbind -b vfio-pci 0000:42:10.2')
...
def dpdk_as(ctx, scen):
    ...
    nmci.run.run('systemctl stop ovsdb-server')
    nmci.run.run('systemctl stop openvswitch')
    time.sleep(5)
    nmci.run.run('nmcli con del dpdk-sriov ovs-iface1 && sleep 1')
    nmci.run.run('systemctl device disconnect p4p1')
...
def wireless_certs_bs(ctx, scen):
    ...
    nmci.run.run('mkdir /tmp/certs')
    if not os.path.isfile('/tmp/certs/eaptest_ca_cert.pem'):
        nmci.run.run('wget http://wlan-lab.eng.bos.redhat.com/certs/eaptest_ca_cert.pem -O /tmp/certs/eaptest_ca_cert.pem')
    if not os.path.isfile('/tmp/certs/client.pem'):
        nmci.run.run('wget http://wlan-lab.eng.bos.redhat.com/certs/client.pem -O /tmp/certs/client.pem')
...
def selinux_allow_ifup_bs(ctx, scen):
    ...
    nmci.run.run("semodule -i tmp/selinux-policy/ifup_policy.pp")
...
def no_testeth10_bs(ctx, scen):
    ...
    nmci.run.run('sudo nmcli connection delete testeth10')
...
def pppoe_bs(ctx, scen):
    ...
    if ctx.arch == "aarch64":
        ...
        nmci.run.run("semodule -i tmp/selinux-policy/pppd.pp")
    print("---------------------------")
    print("installing pppoe dependencies")
    # This -x is to avoid upgrade of NetworkManager in older version testing
    nmci.run.run("rpm -q NetworkManager-ppp || yum -y install NetworkManager-ppp -x NetworkManager")
    nmci.run.run('rpm -q rp-pppoe || yum -y install rp-pppoe')
    nmci.run.run('[ -x //usr/sbin/pppoe-server ] || yum -y install https://kojipkgs.fedoraproject.org//packages/rp-pppoe/3.12/11.fc28/$(uname -p)/rp-pppoe-3.12-11.fc28.$(uname -p).rpm')
    nmci.run.run("mknod /dev/ppp c 108 0")
    nmci.lib.reload_NM_service()
...
def pppoe_as(ctx, scen):
    ...
    nmci.run.run('kill -9 $(pidof pppoe-server)')
    nmci.run.run('nmcli con del ppp ppp2')
...
def del_test1112_veths_bs(ctx, scen):
    ...
    nmci.run.run('''echo 'ENV{ID_NET_DRIVER}=="veth", ENV{INTERFACE}=="test11|test12", ENV{NM_UNMANAGED}="0"' >/etc/udev/rules.d/99-veths.rules''')
    nmci.run.run("udevadm control --reload-rules")
    nmci.run.run("udevadm settle --timeout=5")
    time.sleep(1)
...
def del_test1112_veths_as(ctx, scen):
    ...
    nmci.run.run('ip link del test11')
    nmci.run.run('rm -f /etc/udev/rules.d/99-veths.rules')
    nmci.run.run('udevadm control --reload-rules')
    nmci.run.run('udevadm settle --timeout=5')
    time.sleep(1)
...
def nmstate_setup_bs(ctx, scen):
    ...
    if nmci.run.command_code("sh prepare/nmstate.sh") != 0:
        sys.exit(77)
    ...
    nmci.run.run("ip link set dev eth1 down")
    nmci.run.run("ip link set name ethX eth1")
    nmci.run.run("ip link set dev eth2 down")
    nmci.run.run("ip link set name ethY eth2")

    # We need to have use_tempaddr set to 0 to avoid test_dhcp_on_bridge0 PASSED
    nmci.run.run("echo 0 > /proc/sys/net/ipv6/conf/default/use_tempaddr")

    # Clone default profile but just ipv4 only"
    print("* Rename testeth0 to ipv4 only nmstate")
    nmci.run.run('nmcli connection clone "$(nmcli -g NAME con show -a)" nmstate')
    nmci.run.run("nmcli con modify nmstate ipv6.method disabled ipv6.addresses '' ipv6.gateway ''")
    nmci.run.run("nmcli con up nmstate")

    # Move orig config file to /tmp
    nmci.run.run('mv /etc/NetworkManager/conf.d/99-unmanage-orig.conf /tmp')

    # Remove connectivity packages if present
    nmci.run.run("dnf -y remove NetworkManager-config-connectivity-fedora NetworkManager-config-connectivity-redhat")
...
def nmstate_setup_as(ctx, scen):
    ...
    nmci.run.run("nmcli con del linux-br0 dhcpcli dhcpsrv brtest0 bond99 eth1.101 eth1.102")
    nmci.run.run('nmcli con del eth0 eth1 eth2 eth3 eth4 eth5 eth6 eth7 eth8 eth9 eth10')

    nmci.run.run("nmcli device delete dhcpsrv")
    nmci.run.run("nmcli device delete dhcpcli")
    nmci.run.run("nmcli device delete bond99")

    nmci.run.run("ovs-vsctl del-br ovsbr0")

    # in case of fail we need to kill this
    nmci.run.run('systemctl stop dnsmasq')
    nmci.run.run("pkill -f 'dnsmasq.*/etc/dnsmasq.d/nmstate.conf'")
    nmci.run.run('rm -rf /etc/dnsmasq.d/nmstate.conf')

    # Rename devices back to eth1/eth2
    nmci.run.run("ip link del eth1")
    nmci.run.run("ip link set dev ethX down")
    nmci.run.run("ip link set name eth1 ethX")
    nmci.run.run("ip link set dev eth1 up")

    nmci.run.run("ip link del eth2")
    nmci.run.run("ip link set dev ethY down")
    nmci.run.run("ip link set name eth2 ethY")
    nmci.run.run("ip link set dev eth2 up")


    # remove profiles
    nmci.run.run("nmcli con del nmstate ethX ethY eth1peer eth2peer")

    # Move orig config file to back
    nmci.run.run('mv /tmp/99-unmanage-orig.conf /etc/NetworkManager/conf.d/')
...
def backup_sysconfig_network_bs(ctx, scen):
    ...
    nmci.run.run('sudo cp -f /etc/sysconfig/network /tmp/sysnetwork.backup')


def backup_sysconfig_network_as(ctx, scen):
    print("---------------------------")
    print("restore /etc/sysconfig/network")
    nmci.run.run('sudo mv -f /tmp/sysnetwork.backup /etc/sysconfig/network')
    nmci.run.run('sudo nmcli connection reload')
    nmci.run.run('sudo nmcli connection down testeth9')
...
def remove_fedora_connection_checker_bs(ctx, scen):
    ...
    nmci.lib.wait_for_testeth0()
    nmci.run.run('yum -y remove NetworkManager-config-connectivity-fedora')
    nmci.lib.reload_NM_service()
...
def need_config_server_bs(ctx, scen):
    ...
    if nmci.run.command_code('rpm -q NetworkManager-config-server') == 0:
        ctx.remove_config_server = False
    else:
        nmci.run.run('sudo yum -y install NetworkManager-config-server')
        nmci.lib.reload_NM_service()
...
def need_config_server_as(ctx, scen):
    if ctx.remove_config_server:
        ...
        nmci.run.run('sudo yum -y remove NetworkManager-config-server')
        nmci.lib.reload_NM_service()
...
def no_config_server_bs(ctx, scen):
    ...
    if nmci.run.command_code('rpm -q NetworkManager-config-server') == 1:
        ctx.restore_config_server = False
    else:
        #nmci.run.run('sudo yum -y remove NetworkManager-config-server')
        config_files = nmci.run.command_output('rpm -ql NetworkManager-config-server').strip().split('\n')
        for config_file in config_files:
            config_file = config_file.strip()
            if os.path.isfile(config_file):
                print("* disabling file: %s" % config_file)
                nmci.run.run('sudo mv -f %s %s.off' % (config_file, config_file))
        nmci.lib.reload_NM_service()
    ...
    if ctx.restore_config_server:
        ...
        for config_file in config_files:
            ...
            if os.path.isfile(config_file + '.off'):
                ...
                nmci.run.run('sudo mv -f %s.off %s' % (config_file, config_file))
        nmci.lib.reload_NM_service()
    nmci.run.run("for i in $(nmcli -t -f NAME,UUID connection |grep -v testeth |awk -F ':' ' {print $2}'); do nmcli con del $i; done")
    nmci.lib.restore_testeth0()
...
def permissive_bs(ctx, scen):
    ctx.enforcing = False
    if nmci.run.command_output('getenforce').strip() == 'Enforcing':
        print("---------------------------")
        print("WORKAROUND for permissive selinux")
        ctx.enforcing = True
        nmci.run.run('setenforce 0')
...
def tcpdump_bs(ctx, scen):
    with open("/tmp/network-traffic.log", "w") as f:
        f.write("~~~~~~~~~~~~~~~~~~~~~~~~~~ TRAFFIC LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    nmci.run.Popen("sudo tcpdump -nne -i any >> /tmp/network-traffic.log")


def tcpdump_as(ctx, scen):
    print("Attaching traffic log")
    nmci.run.run("sudo kill -1 $(pidof tcpdump)")
...
def wifi_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id wifi wifi1 qe-open qe-wpa1-psk qe-wpa2-psk qe-wep")
        # nmci.run.run("sudo service NetworkManager restart") # debug restart to overcome the nmcli d w l flickering
    else:
        print("---------------------------")
        print("removing all wifi residues")
        # nmci.run.run('sudo nmcli device disconnect wlan0')
        nmci.run.run('sudo nmcli con del wifi qe-open qe-wep qe-wep-psk qe-wep-enterprise qe-wep-enterprise-cisco')
        nmci.run.run('sudo nmcli con del qe-wpa1-psk qe-wpa2-psk qe-wpa1-enterprise qe-wpa2-enterprise qe-hidden-wpa2-psk')
        nmci.run.run('sudo nmcli con del qe-adhoc qe-ap wifi-wlan0')
        if "novice" in scen.tags:
            ctx.prompt.close()
            time.sleep(1)
            nmci.run.run('sudo nmcli con del wifi-wlan0')
...
def no_connections_bs(ctx, scen):
    ...
    nmci.run.command_code("rm -rf /etc/NetworkManager/system-connections/testeth*")
    nmci.run.command_code("rm -rf /etc/sysconfig/network-scripts/ifcfg-*")
    nmci.run.command_code("nmcli con reload")
...
def bridge_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id bridge0 bridge-slave-eth1 bridge-slave-eth2")
        nmci.lib.reset_hwaddr_nmtui('eth1')
        nmci.lib.reset_hwaddr_nmtui('eth2')
        nmci.run.run("sudo ip link del bridge0")
        ...
        nmci.run.run('sudo nmcli con del bridge4 bridge4.0 bridge4.1 nm-bridge eth4.80 eth4.90')
        nmci.run.run('sudo nmcli con del bridge-slave-eth4 bridge-nonslave-eth4 bridge-slave-eth4.80 eth4')
        nmci.run.run('sudo nmcli con del bridge0 bridge bridge.15 nm-bridge br88 br11 br12 br15 bridge-slave br15-slave br15-slave1 br15-slave2 br10 br10-slave')
...
def vlan_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id vlan eth1.99")
        nmci.run.run("sudo ip link del eth1.99")
        nmci.run.run("sudo ip link del eth2.88")
    else:
        print("---------------------------")
        print("deleting all possible vlan residues")
        nmci.run.run('sudo nmcli con del vlan vlan1 vlan2 eth7.99 eth7.99 eth7.299 eth7.399 eth7.65 eth7.165 eth7.265 eth7.499 eth7.80 eth7.90')
        nmci.run.run('sudo nmcli con del vlan_bridge7.15 vlan_bridge7 vlan_vlan7 vlan_bond7 vlan_bond7.7 vlan_team7 vlan_team7.1 vlan_team7.0')
        nmci.run.run('ip link del bridge7')
        nmci.run.run('ip link del eth7.99')
        nmci.run.run('ip link del eth7.80')
        nmci.run.run('ip link del eth7.90')
        nmci.run.run('ip link del vlan7')
        nmci.run.run('nmcli con down testeth7')
...
def many_vlans_as(ctx, scen):
    ...
    nmci.run.run("for i in {1..255}; do ip link del vlan.$i;done")
...
def bond_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id bond0 bond-slave-eth1 bond-slave-eth2")
        nmci.lib.reset_hwaddr_nmtui('eth1')
        nmci.lib.reset_hwaddr_nmtui('eth2')
        nmci.run.run("sudo ip link del bond0")
        ...
        nmci.run.run('nmcli connection delete id bond0 bond')
        nmci.run.run('ip link del nm-bond')
        nmci.run.run('ip link del bond0')
        #sleep(TIMER)
...
def team_as(ctx, scen):
    if ctx.IS_NMTUI:
        nmci.run.run("sudo nmcli connection delete id team0 team-slave-eth1 team-slave-eth2")
        nmci.lib.reset_hwaddr_nmtui('eth1')
        nmci.lib.reset_hwaddr_nmtui('eth2')
        nmci.run.run("sudo ip link del team0")
        if "many_slaves" in scen.tags:
            nmci.run.run(
        ...
        nmci.run.run('nmcli connection down team0')
        nmci.run.run('nmcli connection delete id team0 team')
        if 'team_assumed' in scen.tags:
            nmci.run.run('ip link del nm-team' )
        #sleep(TIMER)
        nmci.run.run("if nmcli con |grep 'team0 '; then echo 'team0 present: %s' >> /tmp/residues; fi" %scen.tags)
...
def team_slaves_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete id team0.0 team0.1 team-slave-eth5 team-slave-eth6 eth5 eth6 team-slave')
...
def teamd_as(ctx, scen):
    nmci.run.run("systemctl stop teamd")
    nmci.run.run("systemctl reset-failed teamd")
...
def bond_bridge_as(ctx, scen):
    ...
    nmci.run.run('sudo nmcli con del bond_bridge0')
    nmci.run.run('sudo ip link del bond-bridge')
...
def team_br_remove_as(ctx, scen):
    print("---------------------------")
    print("deleting all possible bridge residues")
    nmci.run.run('sudo nmcli con del team_br')
    nmci.run.run('ip link del brA')
...
def gen_br_remove_as(ctx, scen):
    print("---------------------------")
    print("deleting all possible bridge residues")
    nmci.run.run('sudo nmcli con del gen_br')
    nmci.run.run('ip link del brX')
...
def restore_eth1_mtu_as(ctx, scen):
    nmci.run.run("sudo ip link set eth1 mtu 1500")
...
def testeth7_disconnect_as(ctx, scen):
    if ctx.IS_NMTUI:
        if nmci.run.command_code("nmcli connection show -a |grep testeth7") == 0:
...
def clean_iptables_as(ctx, scen):
    ...
    nmci.run.run("iptables -D OUTPUT -p udp --dport 67 -j REJECT")
...
def kill_dhclient_eth8_as(ctx, scen):
    nmci.run.run("kill $(cat /tmp/dhclient_eth8.pid)")
    nmci.run.run("rm -f /tmp/dhclient_eth8.pid")
...
def networking_on_as(ctx, scen):
    ...
    nmci.run.run("nmcli networking on")
    nmci.lib.wait_for_testeth0()
...
def adsl_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id adsl-test11 adsl")
...
def allow_veth_connections_bs(ctx, scen):
    if nmci.run.command_code("grep '^ENV{ID_NET_DRIVER}==\"veth\", ENV{NM_UNMANAGED}=\"1\"' /usr/lib/udev/rules.d/85-nm-unmanaged.rules") == 0:
        nmci.run.run("sed -i 's/^ENV{ID_NET_DRIVER}==\"veth\", ENV{NM_UNMANAGED}=\"1\"/#ENV{ID_NET_DRIVER}==\"veth\", ENV{NM_UNMANAGED}=\"1\"/' /usr/lib/udev/rules.d/85-nm-unmanaged.rules")
...
def allow_veth_connections_as(ctx, scen):
    if ctx.revert_unmanaged:
        nmci.run.run("sed -i 's/^#ENV{ID_NET_DRIVER}==\"veth\", ENV{NM_UNMANAGED}=\"1\"/ENV{ID_NET_DRIVER}==\"veth\", ENV{NM_UNMANAGED}=\"1\"/' /usr/lib/udev/rules.d/85-nm-unmanaged.rules")
        nmci.run.run('sudo rm -rf /etc/NetworkManager/conf.d/99-unmanaged.conf')
        nmci.lib.reload_NM_service()
    nmci.run.run("nmcli con del 'Wired connection 1'")
    nmci.run.run("nmcli con del 'Wired connection 2'")
    nmci.run.run("for i in $(nmcli -t -f DEVICE c s -a |grep -v ^eth0$); do nmcli device disconnect $i; done")
...
def con_ipv4_remove_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id con_ipv4 con_ipv42")
    nmci.run.run("if nmcli con |grep con_ipv4; then echo 'con_ipv4 present: %s' >> /tmp/residues; fi" % scen.tags)
...
def con_ipv6_remove_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection down con_ipv6 ")
    nmci.run.run("nmcli connection down con_ipv62 ")
    nmci.run.run("nmcli connection delete id con_ipv6 con_ipv62")
    nmci.run.run("if nmcli con |grep con_ipv6; then echo 'con_ipv6 present: %s' >> /tmp/residues; fi" %scen.tags)
...
def con_ipv6_ifcfg_remove_as(ctx, scen):
    ...
    #nmci.run.run("nmcli connection delete id con_ipv6 con_ipv62")
    nmci.run.run("rm -rf /etc/sysconfig/network-scripts/ifcfg-con_ipv6")
    nmci.run.run('nmcli con reload')
...
def con_con_remove_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id con_con con_con2")
...
def con_general_remove_as(ctx, scen):
    print("---------------------------")
    print("removing ethernet profiles")
    nmci.run.run("sudo nmcli connection delete id con_general con_general2")
...
def con_tc_remove_as(ctx, scen):
    ...
    nmci.run.run("sudo nmcli connection delete id con_tc")
...
def con_dns_remove_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id con_dns con_dns2")
...
def con_ethernet_remove_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id con_ethernet")
...
def con_vrf_remove_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id vrf.eth4 vrf.eth1 vrf0 vrf1")
...
def con_PBR_remove_as(ctx, scen):
    ...
    nmci.run.run("sudo nmcli connection delete id Servers Internal-Workstations Provider-A Provider-B")
...
def many_con_remove_as(ctx, scen):
    ...
    nmci.run.run('nmcli con del con-team con-bond con-wifi')
...
def gen_bond_remove_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete "Bondy connection 1"')
    print("deleting gen-bond profile")
    nmci.run.run('nmcli connection delete id gen-bond0 gen-bond0.0 gen-bond0.1')
    nmci.run.run('ip link del gen-bond')
    nmci.run.run('ip link del gen-bond0')
...
def general_vlan_as(ctx, scen):
    print("---------------------------")
    print("removing ethernet profiles")
    nmci.run.run("sudo nmcli connection delete id eth8.100")
    nmci.run.run("sudo ip link del eth8.100")
...
def tuntap_as(ctx, scen):
    ...
    nmci.run.run("ip link del tap0")
    nmci.run.run("nmcli con delete tap0")
    nmci.run.run("ip link del brY")
    nmci.run.run("ip link del brX")
...
def slaves_as(ctx, scen):
    ...
    nmci.lib.reset_hwaddr_nmcli('eth4')
    nmci.run.run('nmcli connection delete id bond0.0 bond0.1 bond0.2 bond-slave-eth1 bond-slave')
...
def bond_order_as(ctx, scen):
    ...
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/99-bond.conf")
    nmci.lib.reload_NM_service()
...
def con_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id connie")
    nmci.run.run("rm -rf /etc/sysconfig/network-scripts/ifcfg-connie*")
    #sleep(TIMER)
...
def remove_tombed_connections_as(ctx, scen):
    ...
    for dir in ["/etc/NetworkManager/system-connections/*.nmmeta", "/var/run/NetworkManager/system-connections/*.nmmeta"]:
        try:
            tombs.extend(nmci.run.command_output('ls %s' % dir).split("\n"))
        except Exception:
            pass
    cons = []
    for tomb in tombs:
        ...
        cons.append(con_id)
        nmci.run.run("rm -f %s" % tomb)
    if len(cons):
        nmci.run.run("nmcli con reload")
        nmci.run.run("nmcli con delete %s" % " ".join(cons))
...
def flush_300_as(ctx, scen):
    ...
    nmci.run.run("ip route flush table 300")
...
def stop_radvd_as(ctx, scen):
    ...
    nmci.run.run("sudo systemctl stop radvd")
    nmci.run.run('rm -rf /etc/radvd.conf')
...
def dcb_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id dcb")
...
def mtu_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection modify testeth1 802-3-ethernet.mtu 1500")
    nmci.run.run("nmcli connection up id testeth1")
    nmci.run.run("nmcli connection modify testeth1 802-3-ethernet.mtu 0")
    nmci.run.run("nmcli connection down id testeth1")
    nmci.run.run("ip link set dev eth1 mtu 1500")
    nmci.run.run("ip link set dev eth2 mtu 1500")
    nmci.run.run("ip link set dev eth3 mtu 1500")

    print("---------------------------")
    print("deleting veth devices from mtu test")
    nmci.run.run("nmcli connection delete id tc1 tc2 tc16 tc26")
    nmci.run.run("ip link delete test1")
    nmci.run.run("ip link delete test2")
    nmci.run.run("ip link delete test10")
    nmci.run.run("ip link delete test11")
    nmci.run.run("ip link del vethbr")
    nmci.run.run("ip link del vethbr6")
    nmci.run.run("kill -9 $(ps aux|grep '/usr/sbin/dns' |grep 192.168 |grep -v grep |awk '{print $2}')")
    nmci.run.run("kill -9 $(ps aux|grep '/usr/sbin/dns' |grep 192.168 |grep -v grep |awk '{print $2}')")
...
def mtu_wlan0_as(ctx, scen):
    print("---------------------------")
    print("setting mtu back to 1500")
    nmci.run.run('nmcli con add type wifi ifname wlan0 con-name qe-open autoconnect off ssid qe-open')
    nmci.run.run("nmcli connection modify qe-open 802-11-wireless.mtu 1500")
    nmci.run.run("nmcli connection up id qe-open")
    nmci.run.run("nmcli connection del id qe-open")
...
def macsec_as(ctx, scen):
    print("---------------------------")
    nmci.run.run('sudo nmcli connection delete test-macsec test-macsec-base')
    nmci.run.run('sudo ip netns delete macsec_ns')
    nmci.run.run('sudo ip link delete macsec_veth')
    print("kill wpa_supplicant")
    nmci.run.run("kill $(cat /tmp/wpa_supplicant_ms.pid)")
    print("kill dnsmasq")
    nmci.run.run("kill $(cat /tmp/dnsmasq_ms.pid)")
...
def two_bridged_veths_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id tc1 tc2")
    nmci.run.run("ip link del test1")
    nmci.run.run("ip link del test2")
    nmci.run.run("ip link del vethbr")
    nmci.run.run("nmcli con del tc1 tc2 vethbr")
...
def two_bridged_veths6_as(ctx, scen):
    print("---------------------------")
    print("deleting veth devices")
    nmci.run.run("nmcli connection delete id tc16 tc26 test10 test11 vethbr6")
    nmci.run.run("ip link del test11")
    nmci.run.run("ip link del test10")
    nmci.run.run("ip link del vethbr6")
    nmci.lib.unmanage_veths()
...
def two_bridged_veths_gen_as(ctx, scen):
    print("---------------------------")
    print("deleting veth devices")
    nmci.run.run("ip link del test1g")
    nmci.run.run("ip link del test2g")
    nmci.run.run("ip link del vethbrg")
    nmci.run.run("nmcli con del test1g test2g tc1g tc2g vethbrg")
    time.sleep(1)
...
def dhcpd_as(ctx, scen):
    print("---------------------------")
    print("deleting veth devices")
    nmci.run.run("sudo systemctl stop dhcpd")
...
def modprobe_cfg_remove_as(ctx, scen):
    ...
    nmci.run.run("rm -rf /etc/modprobe.d/99-test.conf")
...
def kill_dnsmasq_vlan_as(ctx, scen):
    print("---------------------------")
    print("kill dnsmasq")
    nmci.run.run("kill $(cat /tmp/dnsmasq_vlan.pid)")
...
def kill_dnsmasq_ip4_as(ctx, scen):
    print("---------------------------")
    print("kill dnsmasq")
    nmci.run.run("kill $(cat /tmp/dnsmasq_ip4.pid)")
...
def kill_dnsmasq_ip6_as(ctx, scen):
    print("---------------------------")
    print("kill dnsmasq")
    nmci.run.run("kill $(cat /tmp/dnsmasq_ip6.pid)")
...
def kill_dhcrelay_as(ctx, scen):
    print("---------------------------")
    print("kill dhcrelay")
    nmci.run.run("kill $(cat /tmp/dhcrelay.pid)")
...
def profie_as(ctx, scen):
    ...
    nmci.run.run("nmcli connection delete id profie")
    #sleep(TIMER)
...
def peers_ns_as(ctx, scen):
    ...
    nmci.run.run("ip netns del peers")
    #sleep(TIMER)
...
def tshark_as(ctx, scen):
    ...
    nmci.run.run("pkill tshark")
    nmci.run.run("rm -rf /etc/dhcp/dhclient-eth*.conf")
...
def mac_as(ctx, scen):
    ...
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/99-mac.conf")
    nmci.lib.reload_NM_service()
    nmci.lib.reset_hwaddr_nmcli('eth1')
...
def keyfile_cleanup_as(ctx, scen):
    ...
    nmci.run.run("sudo sh -c \"rm /usr/lib/NetworkManager/system-connections/*\" ")
    print("removing residual files in /etc/NetworkManager/system-connections")
    nmci.run.run("sudo sh -c \"rm /etc/NetworkManager/system-connections/*\" ")
...
def remove_dns_clean_as(ctx, scen):
    if nmci.run.command_code('grep dns /etc/NetworkManager/NetworkManager.conf') == 0:
        nmci.run.run("sudo sed -i 's/dns=none//' /etc/NetworkManager/NetworkManager.conf")
    nmci.run.run("sudo rm -rf /etc/NetworkManager/conf.d/90-test-dns-none.conf; sleep 1")
    nmci.lib.reload_NM_service()
...
def restore_resolvconf_as(ctx, scen):
    ...
    nmci.run.run('rm -rf /etc/resolv.conf')
    if nmci.run.command_code("systemctl is-active systemd-resolved") == 0:
        nmci.run.run("ln -s /run/systemd/resolve/stub-resolv.conf /etc/resolv.conf")
    nmci.run.run('rm -rf /tmp/resolv_orig.conf')
    nmci.run.run('rm -rf /tmp/resolv.conf')
    nmci.run.run("rm -rf /etc/NetworkManager/conf.d/99-resolv.conf")
    nmci.lib.reload_NM_service()
    nmci.lib.wait_for_testeth0()
...
def device_connect_as(ctx, scen):
    ...
    nmci.run.run('nmcli connection delete testeth9 eth9')
    nmci.run.run('nmcli connection add type ethernet ifname eth9 con-name testeth9 autoconnect no')
...
def restore_eth8_as(ctx, scen):
    ...
    nmci.run.run('sudo ip link del eth8.100')
    nmci.run.run('sudo rm -f /etc/sysconfig/network-scripts/ifcfg-testeth8')
    nmci.run.run('sudo nmcli connection reload')
    nmci.run.run('nmcli connection add type ethernet ifname eth8 con-name testeth8 autoconnect no')
...
def restore_broken_network_as(ctx, scen):
    ...
    nmci.run.run('sudo systemctl stop network.service')
    nmci.run.run('sudo systemctl stop NetworkManager.service')
    nmci.run.run('sysctl net.ipv6.conf.all.accept_ra=1')
    nmci.run.run('sysctl net.ipv6.conf.default.accept_ra=1')
    nmci.lib.restart_NM_service()
    nmci.run.run('sudo nmcli connection down testeth8 testeth9')
...
def add_testeth_as(num):
    def _add_testeth_as(ctx, scen):
        ...
        nmci.run.run('sudo nmcli connection delete eth%d testeth%d' % (num, num))
        nmci.run.run('sudo nmcli connection add type ethernet con-name testeth%d ifname eth%d autoconnect no' % (num, num))
...
def eth_disconnect_as(num):
    def _eth_disconnect_as(ctx, scen):
        ...
        nmci.run.run('sudo nmcli device disconnect eth%d' % num)
        # VVV Up/Down to preserve autoconnect feature
        nmci.run.run('sudo nmcli connection up testeth%d' % num)
        nmci.run.run('sudo nmcli connection down testeth%d' % num)
...
def non_utf_device_as(ctx, scen):
    ...
    if sys.version_info.major < 3:
        nmci.run.run("ip link del $'d\xccf\\c'")
    else:
        nmci.run.run("ip link del $'d\\xccf\\\\c'")
...
def shutdown_as(ctx, scen):
    ...
    nmci.run.run('ip addr del 192.168.50.5/24 dev eth8')
    nmci.run.run('route del default gw 192.168.50.1 eth8')
...
def kill_dbus_monitor_as(ctx, scen):
    ...
    nmci.run.run('pkill -9 dbus-monitor')
...
def restore_rp_filters_as(ctx, scen):
    ...
    nmci.run.run('echo 1 > /proc/sys/net/ipv4/conf/eth2/rp_filter')
    nmci.run.run('echo 1 > /proc/sys/net/ipv4/conf/eth3/rp_filter')
...
def remove_ctcdevice_as(ctx, scen):
    ...
    nmci.run.run("""znetconf -r $(znetconf -c |grep CTC |awk 'BEGIN { FS = "," } ; { print $1 }') -n""")
    time.sleep(1)
