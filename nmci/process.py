...
class _Process:
    ...
    def __init__(self, process_hook=None):
        self.process_hook = process_hook
        self.RunResult = collections.namedtuple("RunResult", ["returncode", "stdout", "stderr"])
        self.IGNORE_RETURNCODE_ALL = object()
    ...
    def _run(
        ...
        if self.process_hook is not None:
            self.process_hook("call", argv, shell, timeout)
        ...
        if self.process_hook is not None:
            self.process_hook("result", argv, returncode, stdout, stderr)
        ...
        if (
            returncode == 0
            or (ignore_returncode is self.IGNORE_RETURNCODE_ALL)
        ...
        return self.RunResult(returncode, stdout, stderr)
