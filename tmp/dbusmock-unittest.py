...
class TestNetworkManager(dbusmock.DBusTestCase):
    ...
    def read_active_connection(self):
        return subprocess.check_output(
            ["nmcli", "--nocheck", "connection", "show", "--active"],
            env=self.lang_env,
            universal_newlines=True,
        )
    ...
    def read_device(self):
        return subprocess.check_output(
            ["nmcli", "--nocheck", "dev"], env=self.lang_env, universal_newlines=True
        )

    def read_device_wifi(self):
        return subprocess.check_output(
            ["nmcli", "--nocheck", "dev", "wifi"],
            env=self.lang_env,
            universal_newlines=True,
        )

    def test_one_eth_disconnected(self):
        self.dbusmock.AddEthernetDevice(
            "mock_Ethernet1", "eth0", DeviceState.DISCONNECTED
        )
    ...
    def test_two_wifi_with_accesspoints(self):
        wifi1 = self.dbusmock.AddWiFiDevice(
            "mock_WiFi1", "wlan0", DeviceState.ACTIVATED
        )
        wifi2 = self.dbusmock.AddWiFiDevice(
            "mock_WiFi2", "wlan1", DeviceState.UNAVAILABLE
        )
        self.dbusmock.AddAccessPoint(
            wifi1,
            "Mock_AP0",
    ...
    def test_wifi_with_connection(self):
        ...
        con1 = self.dbusmock.AddWiFiConnection(
            wifi1, "Mock_Con1", "The_SSID", "wpa-psk"
        )
    ...
    def test_wifi_with_active_connection(self):
        ...
        self.assertEqual(con1, "/org/freedesktop/NetworkManager/Settings/Mock_Con1")
        self.assertEqual(
            active_con1, "/org/freedesktop/NetworkManager/ActiveConnection/Mock_Active1"
        )

        self.assertRegex(self.read_general(), r"connected.*\sfull")
        self.assertRegex(self.read_connection(), r"The_SSID.*\s(802-11-wireless|wifi)")
        self.assertRegex(
            self.read_active_connection(), r"The_SSID.*\s(802-11-wireless|wifi)"
        )
        ...
        self.assertRegex(self.read_connection(), r"The_SSID.*\s(802-11-wireless|wifi)")
        self.assertFalse(
            re.compile(r"The_SSID.*\s(802-11-wireless|wifi)").search(
                self.read_active_connection()
            )
        )
        self.assertRegex(self.read_device_wifi(), "The_SSID")
    ...
    def test_add_connection(self):
        ...
        settings = dbus.Dictionary(
            {
                "connection": dbus.Dictionary(
                    {"id": "test connection", "uuid": uuid, "type": "802-11-wireless"},
                    signature="sv",
                ),
        ...
        self.assertRegex(self.read_general(), r"connected.*\sfull")
        self.assertRegex(
            self.read_connection(), r"%s.*\s(802-11-wireless|wifi)" % uuid2
        )
        self.assertRegex(
            self.read_active_connection(), r"%s.*\s(802-11-wireless|wifi)" % uuid2
        )
    ...
    def test_update_connection(self):
        ...
        settings = dbus.Dictionary(
            {
                "connection": dbus.Dictionary(
                    {"id": "test wireless", "uuid": uuid, "type": "802-11-wireless"},
                    signature="sv",
                ),
                "802-11-wireless": dbus.Dictionary(
                    {"ssid": dbus.ByteArray("The_SSID".encode("UTF-8"))}, signature="sv"
        ...
        new_settings = dbus.Dictionary(
            {
                "connection": dbus.Dictionary(
                    {"id": "test wired", "type": "802-3-ethernet"}, signature="sv"
                ),
                "802-3-ethernet": dbus.Dictionary(
                    {"name": "802-3-ethernet"}, signature="sv"
                ),
            },
            signature="sa{sv}",
        )
        ...
        self.assertRegex(
            self.read_connection(), r"%s.*\s(ethernet|802-3-ethernet)" % uuid
        )
    ...
    def test_remove_connection(self):
        ...
        self.assertRegex(self.read_general(), r"disconnected.*\sfull")
        self.assertFalse(
            re.compile(r"The_SSID.*\s802-11-wireless").search(
                self.read_active_connection()
            )
        )
    ...
    def test_get_conn_by_uuid(self):
        ...
        with self.assertRaisesRegexp(
            dbus.exceptions.DBusException, ".*uuid.*%s$" % fakeuuid
        ):
