...
def main():
    ...
    s_bond = dbus.Dictionary(
        {"interface-name": "nm-bond", "options": {"mode": "1", "miimon": "100"}}
    )
    s_ipv4 = []
    address = "192.168.222.1"
    ip = struct.unpack("=I", socket.inet_pton(socket.AF_INET, address))[0]
    s_ipv4.append(dbus.Array([ip, "24", 0], signature="u"))
    ipv4 = dbus.Dictionary({"addresses": s_ipv4, "method": "manual"}, signature="sv")
    connection = dbus.Dictionary(
        {"bond": s_bond, "ipv4": ipv4, "connection": s_bond_con}
    )
    bus = dbus.SystemBus()
