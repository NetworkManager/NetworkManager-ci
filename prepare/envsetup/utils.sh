...
check_packages () {
    rpm -q iw ethtool wireshark-cli \
           NetworkManager-{openvpn,ppp,pptp,tui,wifi,strongswan} || \
        return 1
...
configure_nm_inf () {
    ...
    # Take just the first infiniband device
    DEV_MASTER=$(nmcli -t -f DEVICE,TYPE device  |grep infiniband |head -n 1 |awk -F ':' '{print $1}')
    echo $DEV_MASTER
...
deploy_ssh_keys () {
    ...
    echo "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEteeNrJ0mr6kjNuYD6/FULzlfPl73obkWeS58yNYGUu eshatsky@redhat.com" >> /root/.ssh/authorized_keys
