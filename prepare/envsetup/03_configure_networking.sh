configure_networking () {
    ...
    if [ $veth -eq 1 ]; then
        ...
        if [ $wlan -eq 1 ]; then
            ...
            mkdir /tmp/certs
            wget --tries=5 --retry-connrefused --retry-on-http-error=404,500,502 --waitretry=2http://tools.lab.eng.brq2.redhat.com:8080/client.pem -O /tmp/certs/client.pem
            wget --tries=5 --retry-connrefused --retry-on-http-error=404,500,502 --waitretry=2http://tools.lab.eng.brq2.redhat.com:8080/ca.pem -O /tmp/certs/eaptest_ca_cert.pem
